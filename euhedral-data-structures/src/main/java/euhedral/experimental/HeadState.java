...
abstract class HeadHolder extends HeadPad {
...
abstract class HeadEndPad extends HeadHolder {
...
abstract class BaseHeadState extends HeadEndPad {

    Object[] headQueue;

    BaseHeadState(Object[] queue) {
        this.headQueue = queue;
    }

}

abstract sealed class HeadState extends BaseHeadState permits ScHeadState {

    protected static final VarHandle HEAD;

    static {
        try {
            HEAD = MethodHandles.lookup().findVarHandle(HeadHolder.class, "head", long.class);
        } catch (Throwable t) {
    ...
    protected final Object[] getHeadQueuePlain() {
        return super.headQueue;
    }

    protected final void setHeadQueuePlain(Object[] queue) {
        super.headQueue = queue;
    }

    protected final void clearHeadQueueSlotPlain(int cIdx) {
        super.headQueue[cIdx] = null;
    }

    protected final long getHeadPlain() {
        return super.head;
    }

    protected final long getHeadAcquire() {
        return (long) HEAD.getAcquire(this);
    }

    protected final void setHeadRelease(long head) {
        HEAD.setRelease(this, head);
    }
}
