...
public final class PartitionedMpscQueue<T> extends AbstractPartitionedQueue<T> {

    private final MpscQueue<T>[] queues;

    public PartitionedMpscQueue(int chunkSize) {
        this(1, chunkSize, 0);
    }

    public PartitionedMpscQueue(int partitions, int chunkSize) {
        this(partitions, chunkSize, 0);
    }

    public PartitionedMpscQueue(int partitions, int chunkSize, int maxPooledChunks) {
        super(partitions);
        this.queues = new MpscQueue[partitions];
        for (int i = 0; i < partitions; i++) {
            this.queues[i] = new MpscQueue<>(chunkSize, maxPooledChunks);
        }
    }
