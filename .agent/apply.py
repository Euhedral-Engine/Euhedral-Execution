...
benchmark_sink = r'''package io.euhedral_execution.training.utils;
...
benchmark_runner = r'''package io.euhedral_execution.training;
...
Path("euhedral-training/src/main/java/io/euhedral_execution/training/utils/BenchmarkFrameSink.java").write_text(benchmark_sink)
Path("euhedral-training/src/main/java/io/euhedral_execution/training/BenchmarkRunner.java").write_text(benchmark_runner)

closed = Path("euhedral-training/src/main/java/io/euhedral_execution/training/ClosedLoopRunner.java")
text = closed.read_text()

def rep(old: str, new: str) -> None:
    global text
    if text.count(old) != 1:
        raise RuntimeError(f"ClosedLoop replacement mismatch: {old[:100]!r}")
    text = text.replace(old, new, 1)

rep('''            Path rawBenchmark = iterationDirectory.resolve("benchmark/raw-data.txt");
...
''')
rep('''                if (!Files.isRegularFile(corpusFile)) {
...
''')
rep('''            rawBenchmark = BenchmarkRunner.run(candidates, rawBenchmark,
...
''')
rep('''            writeState(iterationDirectory, iteration, "COMPLETE", corpusFile);
''', '''            writeState(iterationDirectory, iteration, "COMPLETE", rawBenchmarkDirectory);
''')
rep('''            promote(rawBenchmark, corpusFile);
...
''')
rep('''    private static void promote(Path rawBenchmark, Path corpusFile) throws Exception {
