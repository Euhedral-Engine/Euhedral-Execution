...
public final class MacosResources implements SystemSnapshotProvider {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(Constants.getLoggerName(MacosResources.class));
    ...
    static {
        ...
        try {
            LOCK_STATE = MethodHandles.lookup().findVarHandle(MacosResources.class, "lockState", int.class);
    ...
    private void logZeroDivisionWarning() {
        ...
        if (now - lastZeroDivLoggedNs >= LOG_RATE_LIMIT_NS) {
            LOGGER.warn("Invalid Mach timebase denom (0 or unavailable), falling back to 1:1 tick-to-nanosecond"
                    + " ratio.");
