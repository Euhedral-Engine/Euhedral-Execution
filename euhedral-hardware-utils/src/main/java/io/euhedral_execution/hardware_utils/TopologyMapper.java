...
public final class TopologyMapper {
    ...
    private static boolean sameMembership(EffectiveSocketTopology first,
            EffectiveSocketTopology second) {
        if (first == null || second == null) {
            return first == second;
        }
        return first.effectiveCores.equals(second.effectiveCores)
                && first.effectiveCpus.equals(second.effectiveCpus);
    }
    ...
    TopologyMapper(TopologyModel topologyModel, BitSet allowedCpus) {
        ...
        BitSet ownedAllowed = (BitSet) Objects.requireNonNull(allowedCpus, "allowedCpus").clone();
        ...
        this.effectiveTopology = new EffectiveSystemTopology(new BitSet(), new BitSet(),
                new BitSet(), fixedNullList(topologyModel.maxSocketId() + 1), -1);
    }
    ...
    private void publishIfChanged(BitSet effectiveCpus) {
        ...
        int nextGlobalVersion = previous.globalVersion == -1
                ? 1 : checkedIncrement(previous.globalVersion, "global");
        List<EffectiveSocketTopology> nextSockets = new ArrayList<>(
                Collections.nCopies(topologyModel.maxSocketId() + 1, null));
        ...
        for (int socketId = 0; socketId <= topologyModel.maxSocketId(); socketId++) {
            ...
            EffectiveSocketTopology candidate = buildSocketTopology(socketId, effectiveCpus,
                    effectiveCores, effectiveSockets.get(socketId), 0);
            boolean changed = !sameMembership(priorSocket, candidate);
            if (changed) {
                nextSocketVersions[socketId] = checkedIncrement(nextSocketVersions[socketId],
                        "socket " + socketId);
            }
            if (candidate != null) {
                nextSockets.set(socketId, new EffectiveSocketTopology(
                        nextSocketVersions[socketId], candidate.socketId,
                        candidate.effectiveCores, candidate.effectiveCpus,
                        candidate.effectiveCoreToCpu));
            }
        }

        EffectiveSystemTopology next = new EffectiveSystemTopology(effectiveSockets,
                effectiveCores, effectiveCpus, nextSockets, nextGlobalVersion);
        System.arraycopy(nextSocketVersions, 0, socketVersions, 0, socketVersions.length);
        effectiveTopology = next;
    }

    private EffectiveSocketTopology buildSocketTopology(int socketId, BitSet globalCpus,
            BitSet globalCores, boolean active, int version) {
        if (!active) {
            return null;
        }
    ...
    private List<BitSet> buildCoreToCpus(BitSet cpus) {
        List<BitSet> cores = new ArrayList<>(
                Collections.nCopies(topologyModel.maxCoreId() + 1, null));
    ...
    public record EffectiveSocketTopology(int version, int socketId, BitSet effectiveCores,
        ...
        public EffectiveSocketTopology {
            ...
            List<BitSet> ownedCoreToCpu = new ArrayList<>(
                    Objects.requireNonNull(effectiveCoreToCpu, "effectiveCoreToCpu").size());
    ...
    public record EffectiveSystemTopology(BitSet effectiveSockets,
        ...
        public EffectiveSystemTopology {
            effectiveSockets = new UnmodifiableBitSet(
                    Objects.requireNonNull(effectiveSockets, "effectiveSockets"));
            effectiveCores = new UnmodifiableBitSet(
                    Objects.requireNonNull(effectiveCores, "effectiveCores"));
            effectiveCpus = new UnmodifiableBitSet(
                    Objects.requireNonNull(effectiveCpus, "effectiveCpus"));
