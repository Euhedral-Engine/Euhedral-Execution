...
public final class PressureEvaluator {
    ...
    public static PressureEvaluation evaluate(
            IntervalHardwareSample sample, PressureState priorState, long evaluationNs) {
        ...
        double scopeWaitRaw = evaluateRate(
                sample.scopeSchedulerWaitNs().delta(),
                sample.scopeSchedulerWaitNs().elapsedNs(),
                sample.scopeSchedulerWaitNs().resolution());
        double scopePsiRaw = evaluateRate(
                sample.scopePsiStallNs().delta(),
                sample.scopePsiStallNs().elapsedNs(),
                sample.scopePsiStallNs().resolution());
        double scopeReportedRaw = 0.0;
        if (sample.scopeReportedSchedulerStallRatio().resolution() != SignalResolution.UNAVAILABLE) {
            scopeReportedRaw = unit(sample.scopeReportedSchedulerStallRatio().value());
        }

        double globalThrottleRaw = evaluateRate(
                sample.scopeQuotaThrottledNs().delta(),
        ...
        double smoothedScopeWait = smooth(
                state,
                PressureState.CELL_SCOPE_WAIT,
                scopeWaitRaw,
                sample.scopeSchedulerWaitNs().resolution(),
                evaluationNs);
        double smoothedScopePsi = smooth(
                state,
                PressureState.CELL_SCOPE_PSI,
                scopePsiRaw,
                sample.scopePsiStallNs().resolution(),
                evaluationNs);
        double smoothedScopeReported = smooth(
                state,
                PressureState.CELL_SCOPE_REPORTED,
                scopeReportedRaw,
                sample.scopeReportedSchedulerStallRatio().resolution(),
                evaluationNs);

        double smoothedGlobalThrottle = smooth(
                state,
                PressureState.CELL_GLOBAL_THROTTLE,
                globalThrottleRaw,
                sample.scopeQuotaThrottledNs().resolution(),
                evaluationNs);
        double smoothedSystemThermalLoss = smooth(
                state,
                PressureState.CELL_SYSTEM_THERMAL,
                systemThermalLoss,
                sample.systemSlowSignals().resolution(),
                evaluationNs);
        double smoothedSystemLowPowerLoss = smooth(
                state,
                PressureState.CELL_SYSTEM_LOW_POWER,
                systemLowPowerLoss,
                sample.systemSlowSignals().resolution(),
                evaluationNs);
        ...
        for (int i = 0; i < logicalSpan; i++) {
            ...
            double waitRaw = evaluateRate(
                    sig.schedulerWait().delta(),
                    sig.schedulerWait().elapsedNs(),
                    sig.schedulerWait().resolution());
            double psiRaw = evaluateRate(
                    sig.psiStall().delta(),
                    sig.psiStall().elapsedNs(),
                    sig.psiStall().resolution());
            double reportedRaw = sig.reportedSchedulerStallRatio().resolution() != SignalResolution.UNAVAILABLE
                    ? unit(sig.reportedSchedulerStallRatio().value())
                    : 0.0;

            double runQueueRaw = 0.0;
            if (sig.runnablePerCapacity().resolution() != SignalResolution.UNAVAILABLE) {
                runQueueRaw = unit((sig.runnablePerCapacity().value() - RUN_QUEUE_ONSET) / RUN_QUEUE_RANGE);
            }

            double schedulerRaw =
                    max(scopeWaitRaw, scopePsiRaw, scopeReportedRaw, waitRaw, psiRaw, reportedRaw, runQueueRaw);
            pressurePerCpu[i] = schedulerRaw;

            double waitSmooth = smooth(
                    state,
                    state.cpuCellIndex(i, PressureState.PC_WAIT),
                    waitRaw,
                    sig.schedulerWait().resolution(),
                    evaluationNs);
            double psiSmooth = smooth(
                    state,
                    state.cpuCellIndex(i, PressureState.PC_PSI),
                    psiRaw,
                    sig.psiStall().resolution(),
                    evaluationNs);
            double reportedSmooth = smooth(
                    state,
                    state.cpuCellIndex(i, PressureState.PC_REPORTED),
                    reportedRaw,
                    sig.reportedSchedulerStallRatio().resolution(),
                    evaluationNs);
            double runQueueSmooth = smooth(
                    state,
                    state.cpuCellIndex(i, PressureState.PC_RUN_QUEUE),
                    runQueueRaw,
                    sig.runnablePerCapacity().resolution(),
                    evaluationNs);

            double schedulerSmooth = max(
                    smoothedScopeWait,
                    smoothedScopePsi,
                    smoothedScopeReported,
                    waitSmooth,
                    psiSmooth,
                    reportedSmooth,
                    runQueueSmooth);

            double cpuThrottleRaw = evaluateRate(
                    sig.quotaThrottle().delta(),
                    sig.quotaThrottle().elapsedNs(),
                    sig.quotaThrottle().resolution());
            double cpuThrottleSmooth = smooth(
                    state,
                    state.cpuCellIndex(i, PressureState.PC_CPU_THROTTLE),
                    cpuThrottleRaw,
                    sig.quotaThrottle().resolution(),
                    evaluationNs);
            ...
            double stealRaw = evaluateRate(
                    sig.steal().delta(), sig.steal().elapsedNs(), sig.steal().resolution());
            double stealSmooth = smooth(
                    state,
                    state.cpuCellIndex(i, PressureState.PC_STEAL),
                    stealRaw,
                    sig.steal().resolution(),
                    evaluationNs);
            double externalRaw = sig.externalContentionRatio().resolution() != SignalResolution.UNAVAILABLE
                    ? unit(sig.externalContentionRatio().value())
                    : 0.0;
            double externalSmooth = smooth(
                    state,
                    state.cpuCellIndex(i, PressureState.PC_EXTERNAL),
                    externalRaw,
                    sig.externalContentionRatio().resolution(),
                    evaluationNs);
            ...
            double capacityLossSmooth = smooth(
                    state,
                    state.cpuCellIndex(i, PressureState.PC_CAPACITY_LOSS),
                    capacityLossRaw,
            ...
            double freqLossSmooth = smooth(
                    state,
                    state.cpuCellIndex(i, PressureState.PC_FREQUENCY_LOSS),
                    freqLossRaw,
            ...
            double perCpuThermalSmooth = smooth(
                    state,
                    state.cpuCellIndex(i, PressureState.PC_THERMAL),
                    perCpuThermalLossRaw,
                    slow.resolution(),
                    evaluationNs);

            double perCpuLowPowerLossRaw = lowPowerLoss(slow.lowPowerMode());
            double perCpuLowPowerSmooth = smooth(
                    state,
                    state.cpuCellIndex(i, PressureState.PC_LOW_POWER),
                    perCpuLowPowerLossRaw,
                    slow.resolution(),
                    evaluationNs);

            double capacityDomain = max(
                    capacityLossSmooth,
                    freqLossSmooth,
                    perCpuThermalSmooth,
                    smoothedSystemThermalLoss,
                    perCpuLowPowerSmooth,
                    smoothedSystemLowPowerLoss);
        ...
        long usageBytes = mem.usageBytes().resolution() != SignalResolution.UNAVAILABLE
                ? Math.max(0, mem.usageBytes().value())
                : 0L;
        long inactiveBytes = mem.inactiveFileBytes().resolution() != SignalResolution.UNAVAILABLE
                ? Math.max(0, mem.inactiveFileBytes().value())
                : 0L;
        ...
        if (L >= 0) {
            ...
            headroomSmooth =
                    smooth(state, PressureState.CELL_HEADROOM, headroomRaw, SignalResolution.CURRENT, evaluationNs);
        } else {
            headroomSmooth =
                    smooth(state, PressureState.CELL_HEADROOM, 0.0, SignalResolution.UNAVAILABLE, evaluationNs);
        }

        double reclaimRaw = 0.0;
        if (L > 0
                && mem.cumulativeReclaimBytes().resolution() != SignalResolution.UNAVAILABLE
                && mem.cumulativeReclaimBytes().resolution() != SignalResolution.BASELINE) {
            double reclaimFrac = ((double) mem.cumulativeReclaimBytes().delta() * 1_000_000_000.0)
                    / (L * mem.cumulativeReclaimBytes().elapsedNs());
            if (Double.isFinite(reclaimFrac)) {
                reclaimRaw = unit(reclaimFrac / RECLAIM_FULL_FRACTION);
            }
        }
        reclaimSmooth = smooth(
                state,
                PressureState.CELL_RECLAIM,
                reclaimRaw,
                L > 0 ? mem.cumulativeReclaimBytes().resolution() : SignalResolution.UNAVAILABLE,
                evaluationNs);

        double memStallRaw = evaluateRate(
                mem.memoryStallNs().delta(),
                mem.memoryStallNs().elapsedNs(),
                mem.memoryStallNs().resolution());
        memoryStallSmooth = smooth(
                state,
                PressureState.CELL_MEMORY_STALL,
                memStallRaw,
                mem.memoryStallNs().resolution(),
                evaluationNs);
        ...
        double ioStallRaw = evaluateRate(
                io.stallNs().delta(), io.stallNs().elapsedNs(), io.stallNs().resolution());
        double ioStallSmooth = smooth(
                state, PressureState.CELL_IO_STALL, ioStallRaw, io.stallNs().resolution(), evaluationNs);
        ...
        if (io.operationsLatency().resolution() != SignalResolution.UNAVAILABLE
            ...
            if (io.operationsLatency().operationsDelta() > 0) {
                double avgLatencyNs = (double) io.operationsLatency().latencyDelta()
                        / io.operationsLatency().operationsDelta();
                if (Double.isFinite(avgLatencyNs)) {
                    latencyRaw = unit((avgLatencyNs - IO_LATENCY_ONSET_NS) / IO_LATENCY_RANGE_NS);
                }
            }
        }
        double latencySmooth = smooth(
                state,
                PressureState.CELL_IO_LATENCY,
                latencyRaw,
                io.operationsLatency().resolution(),
                evaluationNs);
        ...
        double queueSmooth = smooth(
                state,
                PressureState.CELL_IO_QUEUE,
                queueRaw,
                io.maximumQueueDepth().resolution(),
                evaluationNs);
        ...
        if (io.productiveBytes().resolution() != SignalResolution.UNAVAILABLE
            ...
            double rate = ((double) io.productiveBytes().delta() * 1_000_000_000.0)
                    / io.productiveBytes().elapsedNs();
    ...
    private static double smooth(
            PressureState state, int cellIndex, double input, SignalResolution res, long evaluationNs) {
        ...
        double tau = (input >= state.previous[cellIndex]) ? ATTACK_TAU_SECONDS_INV : RELEASE_TAU_SECONDS_INV;
