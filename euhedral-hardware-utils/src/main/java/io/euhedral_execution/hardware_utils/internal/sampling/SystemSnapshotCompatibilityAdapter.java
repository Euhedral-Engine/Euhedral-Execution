...
/// DetailedSystemSnapshotProvider SPI by performing one-time profile detection and producing
/// signal-validity-annotated fast and slow samples.
///
/// Construction: the profile is detected once at construction from the concrete type of the
/// delegate via instanceof. The profile is fixed for the lifetime of the adapter. No dynamic
/// profile change is possible.
///
/// A construction-time call to delegate.getSnapshot() is made once to capture logicalSpan from
/// snap.totalCpus(). This is necessary because sampleSlow() must produce a CpuSlowSignals[] of
/// exactly logicalSpan entries without consulting the delegate again. If the initial snapshot is
/// null, construction fails with IllegalStateException because logicalSpan cannot be determined
/// safely. This is the only construction-time failure mode beyond a null delegate.
///
/// sampleFast semantics: Calls delegate.getSnapshot() exactly once per invocation. All valid signal
/// observedAtNs values are set to snap.timeNs() from that one call. A null snapshot is a transient
/// failure: all signals are returned as TRANSIENT_FAILURE in a complete FastHardwareSample at
/// requestedAtNs.
///
/// sampleSlow semantics: Returns an all-UNSUPPORTED SlowHardwareSample at requestedAtNs without
/// calling the delegate. Legacy providers cannot supply slow signals.
///
/// Period conversion (C1 fix): LINUX_V2_LEGACY: snap.period() is in microseconds; multiplied by
/// 1000 with Math.multiplyExact after rejecting a negative input. Overflow returns transient
/// failure. CANONICAL_PUBLIC/WINDOWS_LEGACY/MACOS_LEGACY: snap.period() is in nanoseconds and is
/// used directly.
///
/// Null profile restriction: DetailedSystemSnapshotProvider delegates must not be wrapped; use
/// wrap() which returns the delegate directly in that case.
public final class SystemSnapshotCompatibilityAdapter implements DetailedSystemSnapshotProvider {
  ...
  /// Fixed logical CPU count captured from the first getSnapshot() call at construction. Used to
  /// produce correctly sized CpuSlowSignals[] in sampleSlow().
  private final int logicalSpan;

  /// Constructs an adapter for a non-DetailedSystemSnapshotProvider delegate.
  ///
  /// Throws NullPointerException if delegate is null. Throws IllegalArgumentException if delegate
  /// is already a DetailedSystemSnapshotProvider. Throws IllegalStateException if the initial
  /// getSnapshot() returns null (logicalSpan cannot be determined).
  public SystemSnapshotCompatibilityAdapter(SystemSnapshotProvider delegate) {
    ...
    } else if (delegate instanceof MacosResources) {
  ...
  /// Calls delegate.getSnapshot() exactly once and maps the result to a FastHardwareSample with
  /// per-profile signal validity annotations.
  ///
  /// A null snapshot is a transient failure: returns a complete FastHardwareSample with every
  /// signal set to TRANSIENT_FAILURE at requestedAtNs (M2 fix).
  ///
  /// Period conversion (C1 fix): LINUX_V2_LEGACY -- snap.period() is in microseconds. A negative
  /// period is rejected before multiplication (M5 fix). Overflow from Math.multiplyExact returns
  /// transient-failure period. All other profiles -- snap.period() is in nanoseconds; used
  /// directly. A negative nanosecond period maps to transient-failure period.
  @Override
  public FastHardwareSample sampleFast(long requestedAtNs) {
    ...
    if (profile == CompatibilityProfile.WINDOWS_LEGACY
        || profile == CompatibilityProfile.MACOS_LEGACY) {
    ...
    MemoryFastSignals memory =
        new MemoryFastSignals(
            hardLimit,
  ...
  /// Returns an all-UNSUPPORTED SlowHardwareSample at requestedAtNs without invoking the wrapped
  /// provider. Legacy providers cannot supply slow signals.
  @Override
  public SlowHardwareSample sampleSlow(long requestedAtNs) {
    ...
    return new SlowHardwareSample(
        requestedAtNs, logicalSpan, cpuSlow, createUnsupportedSystemSlow(requestedAtNs));
  }
  ...
  ///
  /// Rejects negative input before calling Math.multiplyExact (M5 fix): periodMicros < 0 ->
  /// transient failure (not a valid period). overflow -> transient failure. 0 -> unsupported
  /// (unavailable/not applicable). positive -> valid LongGaugeSignal in nanoseconds.
  ...
  /// Builds a complete FastHardwareSample with every signal set to TRANSIENT_FAILURE, used when the
  /// delegate returns a null snapshot.
  ///
  /// requestedAtNs is used as the observedAtNs for all signals and for the outer sample itself,
  /// matching the spec's "transient sample failure" rule.
