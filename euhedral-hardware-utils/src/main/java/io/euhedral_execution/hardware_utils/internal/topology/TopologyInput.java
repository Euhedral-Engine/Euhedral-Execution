...
public record TopologyInput(String providerName, List<LogicalCpu> logicalCpus, List<CacheDomain> cacheDomains) {
    ...
    public TopologyInput {
        ...
        cacheDomains = Objects.requireNonNull(cacheDomains, "cacheDomains").stream()
                .map(domain -> new CacheDomain(
                        domain.level(), domain.sizeBytes(), domain.lineSizeBytes(), domain.logicalCpuSharers()))
                .toList();
    }
}
