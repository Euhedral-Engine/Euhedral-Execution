...
PAIRS_TSV_COLUMNS = [
  ...
  "labelEvidenceBasis",
    ...
    "pairWeight",
  "basisThroughput_K",
  "basisThroughput_KMinus1",
  "basisDeltaThroughput",
  "basisVariance_K",
  "basisVariance_KMinus1",
  "basisUncertainty",
    "kRunPath",
    "kRunSha256",
    "kMinus1RunPath",
    ...
    with open(output_path, "w", encoding="utf-8", newline="") as f:
        ...
        for p in pairs:
            writer.writerow([
              ...
              p.effective_outcome.value,
              p.label_evidence_basis.value,
                f"{p.y:.1f}",
                f"{p.pair_weight:.6f}",
              f"{p.basis_throughput_k:.3f}",
              f"{p.basis_throughput_k_minus_1:.3f}",
              f"{p.basis_delta:.3f}",
              f"{p.basis_variance_k:.3f}",
              f"{p.basis_variance_k_minus_1:.3f}",
              f"{p.basis_uncertainty:.3f}",
                str(p.k_run_path),
