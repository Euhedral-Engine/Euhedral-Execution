...
public final class RunAggregator {
    ...
    public static List<RunAggregate> aggregate(List<Path> bundles, PolicyRegistry policies, AggregationConfig config) {
        List<Path> sorted = bundles.stream()
        ...
        result.sort(Comparator.comparing(
                        (RunAggregate row) -> row.run().descriptor().scenario())
                .thenComparing(row -> row.policy().id())
                .thenComparing(row -> row.run().descriptor().benchmarkRunId()));
        return List.copyOf(result);
    }

    private static RunAggregate aggregate(
            PolicyAccumulator accumulator, BenchmarkRunContext run, AggregationConfig config) {
        ...
        if (!successes.isEmpty()) {
            double[] values =
                    successes.stream().mapToDouble(Double::doubleValue).toArray();
        ...
        return new RunAggregate(
                ...
                accumulator.skipped,
                successRate,
                accumulator.timeout / (double) planned,
    ...
    private static final class BundleAccumulator implements ObservationBundleReader.ObservationVisitor {
        ...
        private BundleAccumulator(PolicyRegistry registry, AggregationConfig config, Set<ObservationKey> globalKeys) {
        ...
        private List<RunAggregate> finish() {
            return policies.values().stream()
                    .map(policy -> aggregate(policy, run, config))
                    .toList();
        }
    }
