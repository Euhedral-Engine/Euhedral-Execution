...
public final class MergeRecords {
    ...
    public enum RunAggregateStatus {
        ...
        LOW_SUCCESS_FRACTION,
        NONPOSITIVE_THROUGHPUT
    }
    ...
    public record RunAggregate(
        ...
        public RunAggregate {
            ...
            TreeSet<PolicyRole> sortedRoles = new TreeSet<>(java.util.Comparator.comparing(Enum::name));
            ...
            if (plannedRepetitionCount != run.descriptor().parameters().expectedRepetitions()
                    || successfulRepetitionCount < 0
                    || timeoutCount < 0
                    || failedCount < 0
                    || skippedCount < 0
                    || successfulRepetitionCount + timeoutCount + failedCount + skippedCount != plannedRepetitionCount
                    || Double.compare(successRate, successfulRepetitionCount / (double) plannedRepetitionCount) != 0
                    || Double.compare(timeoutRate, timeoutCount / (double) plannedRepetitionCount) != 0
                    || Double.compare(failureRate, (failedCount + skippedCount) / (double) plannedRepetitionCount) != 0
                    || Double.compare(
                                    nonSuccessRate,
                                    (timeoutCount + failedCount + skippedCount) / (double) plannedRepetitionCount)
                            != 0) {
                throw new IllegalArgumentException("Invalid run aggregate counts or rates");
            }
            boolean statisticsPresent = rawP25.isPresent()
                    && rawMedian.isPresent()
                    && rawP75.isPresent()
                    && rawIqr.isPresent()
                    && rawLogIqr.isPresent();
            if (successfulRepetitionCount == 0
                    ? statisticsPresent
                    : !statisticsPresent
                            || !finitePositive(rawP25)
                            || !finitePositive(rawMedian)
                            || !finitePositive(rawP75)
                            || !finiteNonNegative(rawIqr)
                            || !finiteNonNegative(rawLogIqr)
                            || rawP25.getAsDouble() > rawMedian.getAsDouble()
                            || rawMedian.getAsDouble() > rawP75.getAsDouble()
                            || Double.compare(rawIqr.getAsDouble(), rawP75.getAsDouble() - rawP25.getAsDouble()) != 0) {
    ...
    public record RunCalibration(
        ...
        public RunCalibration {
            ...
            cappedAnchorWeights = Collections.unmodifiableSortedMap(new TreeMap<>(cappedAnchorWeights));
            if (!anchorSetId.matches("a1-[0-9a-f]{16}")
                    || fixedAnchorCount < 1
                    || sharedAnchorCount < 0
                    || sharedAnchorCount > fixedAnchorCount
                    || reason.isEmpty()
                    || !cappedAnchorWeights.isEmpty() && cappedAnchorWeights.size() != sharedAnchorCount
    ...
    public record ScenarioResult(
        ...
        public ScenarioResult {
            ...
            if (acceptedRunCount < 0
                    || acceptedRunCount > totalRunCount
                    || weakRunCount < 0
            ...
            boolean numerics = throughputP25.isPresent()
                    && throughputMedian.isPresent()
                    && throughputP75.isPresent()
                    && throughputIqr.isPresent()
                    && medianWithinRunRelativeIqr.isPresent()
                    && meanTimeoutRate.isPresent()
                    && meanFailureRate.isPresent()
                    && meanNonSuccessRate.isPresent()
                    && bootstrapMedianCiLow.isPresent()
                    && bootstrapMedianCiHigh.isPresent();
            if (valid != numerics || quality.isPresent() && !valid) {
                throw new IllegalArgumentException("Scenario status and numerics disagree");
            }
            if (valid
                    && (!finitePositive(throughputP25)
                            || !finitePositive(throughputMedian)
                            || !finitePositive(throughputP75)
                            || !finiteNonNegative(throughputIqr)
                            || !finiteNonNegative(medianWithinRunRelativeIqr)
                            || !rate(meanTimeoutRate)
                            || !rate(meanFailureRate)
                            || !rate(meanNonSuccessRate)
                            || !finitePositive(bootstrapMedianCiLow)
        ...
        public ScenarioResult withQuality(double newQuality) {
            return new ScenarioResult(
                    ...
                    plannedRepetitionCount,
                    throughputP25,
                    throughputMedian,
                    throughputP75,
                    throughputIqr,
                    medianWithinRunRelativeIqr,
                    meanTimeoutRate,
                    meanFailureRate,
                    meanNonSuccessRate,
                    bootstrapMedianCiLow,
                    bootstrapMedianCiHigh,
    ...
    public record RobustPolicySummary(
        ...
        public RobustPolicySummary {
            ...
            if (validRequiredScenarioCount < 0
                    ...
                    || Double.compare(coverageFraction, validRequiredScenarioCount / (double) requiredScenarioCount)
                            != 0
                    || eligible != (validRequiredScenarioCount == requiredScenarioCount)) {
                throw new IllegalArgumentException("Coverage counts disagree");
            }
            if (eligible
                    && (worstQuality.isEmpty()
                            || qualityP25.isEmpty()
                            || geometricMeanQuality.isEmpty()
                            || crossScenarioQualityMad.isEmpty()
                            || medianRelativeIqr.isEmpty()
                            || meanNonSuccessRate.isEmpty()
            ...
            if (eligible
                    && (!rate(worstQuality)
                            || !rate(qualityP25)
                            || !rate(geometricMeanQuality)
                            || !rate(crossScenarioQualityMad)
                            || !finiteNonNegative(medianRelativeIqr)
                            || !rate(meanNonSuccessRate)
                            || !rate(meanTimeoutRate))) {
