...
public record ReferenceRunCatalog(int schemaVersion, String anchorSetId,
    ...
    public ReferenceRunCatalog {
        ...
        if (schemaVersion != 1 || anchorSetId == null
                || !anchorSetId.matches("a1-[0-9a-f]{16}") || referenceRunIds.isEmpty()) {
            throw new IllegalArgumentException("Invalid reference catalog");
        }
        if (referenceRunIds.values().stream().anyMatch(runId -> runId == null
                || !runId.matches("[a-z0-9][a-z0-9._-]{0,95}"))
                || referenceRunIds.values().stream().distinct().count()
