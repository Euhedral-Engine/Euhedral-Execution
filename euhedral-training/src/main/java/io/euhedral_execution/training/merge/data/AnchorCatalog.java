...
public record AnchorCatalog(int schemaVersion, String anchorSetId, List<PolicyVector> fixedAnchors) {

    public AnchorCatalog {
        fixedAnchors = fixedAnchors.stream()
    ...
    public static AnchorCatalog of(List<PolicyVector> fixedAnchors) {
        List<PolicyVector> sorted = fixedAnchors.stream()
                .sorted(Comparator.comparing(PolicyVector::id))
                .toList();
    ...
    private static String computedId(List<PolicyVector> anchors) {
        ...
        return "a1-" + String.format("%016x", HasherApi.getHash(input.toString().getBytes(StandardCharsets.UTF_8)));
    }
}
