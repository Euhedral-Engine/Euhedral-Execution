...
public record AnchorSelectionConfig(double fixedFraction, int minimumFixedAnchors,
    ...
    public AnchorSelectionConfig {
        if (!(fixedFraction > 0 && fixedFraction <= 1) || minimumFixedAnchors < 1
                || !Double.isFinite(maximumBootstrapNonSuccessRate) || !Double.isFinite(
                maximumBootstrapRelativeIqr) || maximumBootstrapNonSuccessRate < 0
                || maximumBootstrapNonSuccessRate > 1 || maximumBootstrapRelativeIqr < 0) {
    ...
    public int targetCount(int policyBudget) {
        int target =
                Math.max(minimumFixedAnchors, (int) StrictMath.ceil(fixedFraction * policyBudget));
        if (policyBudget <= target) {
            throw new IllegalArgumentException("Budget must exceed anchors");
        }
        return target;
    }
}
