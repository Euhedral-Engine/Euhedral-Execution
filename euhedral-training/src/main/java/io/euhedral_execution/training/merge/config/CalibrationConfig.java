...
public record CalibrationConfig(int minimumStrongAnchors, int minimumWeakAnchors,
                                double maximumStrongResidual, double maximumWeakResidual,
                                double minimumLogSigma, double maximumAnchorWeightShare) {

    public CalibrationConfig {
        if (minimumStrongAnchors < minimumWeakAnchors || minimumWeakAnchors < 1 || !Double.isFinite(
                maximumStrongResidual) || !Double.isFinite(maximumWeakResidual) || !Double.isFinite(
                minimumLogSigma) || !Double.isFinite(maximumAnchorWeightShare)
