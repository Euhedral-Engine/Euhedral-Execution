...
final class PackageReportWriter {
    ...
    static void write(PackageSourceSet source, TrainingRunPackageInputs inputs, Path root) throws IOException {
        CanonicalFileSupport.write(root.resolve("README.md"), readme(source, inputs));
        CanonicalFileSupport.write(root.resolve("reports/robust-ranking.md"), ranking(source));
        CanonicalFileSupport.write(root.resolve("reports/source-scenario-comparison.md"), scenarios(source));
    }

    private static String readme(PackageSourceSet source, TrainingRunPackageInputs inputs) throws IOException {
        var checkpoint = source.loaded().checkpoint();
        StringBuilder out = new StringBuilder("# Euhedral training run ")
                .append(checkpoint.trainingRunId())
                .append("\n\n## Status\n\n")
                .append("- Checkpoint stage: `")
                .append(checkpoint.stage())
                .append("`\n")
                .append("- Checkpoint revision: ")
                .append(checkpoint.revision())
                .append("\n")
        ...
        if (source.omissions().isEmpty()) out.append("- Omissions: none\n");
            ...
            for (PackageOmission omission : source.omissions()) {
                out.append("- Omission `")
                        .append(omission.semanticGroup())
                        .append("`: `")
                        .append(omission.reason())
                        .append("`\n");
            }
        out.append("\n## Winning policies\n\n");
        if (source.merge() == null || source.winners().isEmpty()) {
            ...
            List<List<String>> ranking = CanonicalCsv.read(source.merge().resolve("robust-ranking.csv"));
            ...
            for (int index = 1; index < ranking.size() && emitted < 10; index++) {
                ...
                out.append("| ")
                        .append(cell(row.get(1)))
                        .append(" | ")
                        .append(cell(row.get(2)))
                        .append(" | ")
                        .append(cell(row.get(8)))
                        .append(" | ")
                        .append(cell(row.get(9)))
                        .append(" | ")
                        .append(cell(row.get(10)))
                        .append(" | ")
                        .append(cell(row.get(11)))
                        .append(" | ")
                        .append(cell(row.get(13)))
                        .append(" | ")
                        .append(cell(row.get(14)))
                        .append(" |\n");
                emitted++;
            }
        ...
        checkpoint
                .requiredScenarios()
                .forEach(scenario -> out.append("| ")
                        .append(cell(scenario.canonical()))
                        .append(" | ")
                        .append(cell(scenario.environmentId()))
                        .append(" | ")
                        .append(scenario.sourceCount())
                        .append(" | ")
                        .append(scenario.availablePhysicalCoreCount())
                        .append(" | ")
                        .append(scenario.ratio().numerator())
                        .append('/')
                        .append(scenario.ratio().denominator())
                        .append(" |\n"));
        ...
        if (source.merge() == null) {
            ...
            out.append("Counts describe benchmark runs, not candidate policies. Reference runs "
                    ...
                    .append(counts.getOrDefault("REFERENCE", 0))
                    .append("\n")
                    ...
                    .append(counts.getOrDefault("UNCALIBRATED", 0))
                    .append("\n")
                    .append("- Acceptance mode: `")
                    .append(source.calibrationAcceptance())
                    .append("`\n");
        ...
        if (source.modelMetadata() == null) {
            ...
            boolean accepted = source.modelMetadata().acceptanceStatus() == ModelAcceptanceStatus.ACCEPTED;
            out.append("Model status: `")
                    .append(accepted ? "accepted/deployable" : "rejected")
                    .append("`. Dataset fingerprint: `")
                    .append(source.modelMetadata().datasetFingerprintSha256())
                    .append("`.");
        ...
        out.append("\n## Package guide\n\n"
                        ...
                        + "## Provenance\n\n"
                        + "- Producer commit: `")
                .append(inputs.commitSha())
                .append("`\n" + "- Dirty working tree: ")
                .append(inputs.dirtyWorkingTree())
                .append("\n")
                .append("- Evidence: ")
                .append(originCounts(source))
                .append("\n")
                .append("- Config SHA-256: `")
                .append(checkpoint.configSha256())
                .append("`\n")
                .append("- Checkpoint SHA-256: `");
        out.append(
                source.loaded().snapshotDirectory() == null
                        ? "n/a"
    ...
    private static String ranking(PackageSourceSet source) throws IOException {
        ...
        if (source.merge() == null)
            return out.append("Ranking is unavailable.\n").toString();
        List<List<String>> rows = CanonicalCsv.read(source.merge().resolve("robust-ranking.csv"));
        ...
        for (int index = 1; index < rows.size(); index++) {
            ...
            if (!row.get(3).equals("true")) {
                out.append("| ")
                        .append(cell(row.get(2)))
                        .append(" | ")
                        .append(cell(row.get(6)))
                        .append(" | ")
                        .append(cell(row.get(5)))
                        .append(" | ")
                        .append(cell(row.get(4)))
                        .append(" | ")
                        .append(cell(row.get(15)))
                        .append(" |\n");
            }
        }
        return out.toString();
    ...
    private static Map<String, double[]> winnerVectors(Path merge) throws IOException {
        List<List<String>> rows = CanonicalCsv.read(merge.resolve("robust-leaders.vectors.csv"));
        if (rows.isEmpty()) {
        ...
        for (int index = 1; index < rows.size(); index++) {
            ...
            for (int lane = 0; lane < vector.length; lane++) {
                vector[lane] = Double.longBitsToDouble(Long.parseUnsignedLong(row.get(weightColumn + lane), 16));
    ...
    private static void appendVectors(StringBuilder out, List<String> policies, Map<String, double[]> vectors) {
    ...
    private static String scenarios(PackageSourceSet source) throws IOException {
        ...
        List<List<String>> rows = CanonicalCsv.read(source.merge().resolve("scenario-results.csv"));
            ...
            for (String policy : source.winners()) {
                ...
                if (row == null) out.append(" | n/a | n/a | n/a | n/a | n/a | n/a | n/a | n/a |\n");
                    ...
                    out.append(" | ")
                            ...
                            .append(cell(row.get(15)))
                            .append(" | ")
                            .append(cell(row.get(16)))
                            .append(" | ")
                            .append(cell(row.get(17)))
                            .append(" | ")
                            .append(cell(row.get(25)))
                            .append(" | ")
                            .append(cell(row.get(19)))
                            .append(" | ")
                            .append(cell(row.get(22)))
                            .append(" | ")
                            .append(cell(row.get(20)))
                            .append(" |\n");
            }
        }
        return out.toString();
    ...
    private static String originCounts(PackageSourceSet source) {
        long nativeCount = source.evidence().stream()
                .filter(item -> item.origin() == EvidenceOrigin.NATIVE)
                .count();
        long imported = source.evidence().size() - nativeCount;
        return "native=" + nativeCount + ", imported=" + imported + ", mixed="
                + (nativeCount > 0 && imported > 0 ? 1 : 0);
    }
