...
public final class TrainingRunPackager {
    private static final List<String> MERGE_COPIES = List.of("fixed-anchors.csv",
    ...
    public static TrainingRunPackage publish(TrainingRunPackageRequest request)
        ...
        try (WorkspaceLock ignored = WorkspaceLock.acquire(request.workspace())) {
            ...
            try {
                ...
                return new TrainingRunPackage(target, target.resolve("manifest.json"),
                        intended.packageId(), intended.status());
    ...
    private static void stage(PackageSourceSet source, TrainingRunPackageInputs inputs,
        ...
        if (source.merge() != null) {
            ...
            CanonicalFileSupport.copy(source.merge().resolve("robust-leaders.vectors.csv"),
                    staging.resolve("vectors/robust-leaders.vectors.csv"));
            CanonicalFileSupport.copy(source.merge().resolve("incomplete-policies.vectors.csv"),
                    staging.resolve("vectors/incomplete-promising.vectors.csv"));
            PackageDatasetWriter.writeMeasurements(source.merge(),
                    staging.resolve("policy-scenario-measurements.csv"));
        }
        ...
        if (source.schedule() != null) {
            ...
            PackageDatasetWriter.writeBenchmarkReady(source,
                    staging.resolve("vectors/benchmark-ready.vectors.csv"));
        }
        for (PackageSourceSet.EvidenceInfo evidence : source.evidence()) {
            CanonicalFileSupport.copyDirectory(evidence.directory(),
        ...
        CanonicalFileSupport.write(staging.resolve("provenance/package-inputs.properties"),
                TrainingRunPackageInputsCodec.encode(inputs));
        PackageReportWriter.write(source, inputs, staging);
    }

    private static TrainingRunManifest manifest(PackageSourceSet source,
            TrainingRunPackageInputs inputs, Path staging) throws IOException {
        ...
        try (var stream = Files.walk(staging)) {
            for (Path path : stream.filter(item ->
                    ...
                    .sorted(Comparator.comparing(item ->
                            staging.relativize(item).toString().replace('\\', '/'))).toList()) {
                String relative = staging.relativize(path).toString().replace('\\', '/');
                if (relative.equals("manifest.json")
                        || relative.equals(".euhedral-package-staging")) continue;
                files.add(file(source, path, relative));
            }
        }
        var checkpoint = source.loaded().checkpoint();
        return new TrainingRunManifest(inputs.packageId(), inputs.trainingRunId(),
                inputs.checkpointRevision(), checkpoint.stage(), source.status(),
                ...
                inputs.commitSha(), inputs.dirtyWorkingTree(),
                List.copyOf(checkpoint.requiredScenarios()), source.calibrationAcceptance(),
                source.winners(), files, source.omissions());
    }

    private static PackageFile file(PackageSourceSet source, Path file, String relative)
            throws IOException {
        Classification classification = classify(relative);
        CanonicalFileSupport.CsvMetadata csv = relative.endsWith(".csv")
                ? CanonicalFileSupport.csvMetadata(file) : null;
        List<String> runIds = sourceRuns(source, relative);
        return new PackageFile(relative, classification.semanticType(),
                mediaType(relative), csv == null ? null : csv.schemaVersion(),
                csv == null ? null : csv.rowCount(), CanonicalFileSupport.sha256(file),
                classification.producingStage(), runIds, origin(source, runIds),
                true);
    }

    static Classification classify(String path) {
        if (path.equals("README.md")) return c(ArtifactSemanticType.PACKAGE_README,
                ProducingStage.PACKAGE);
        if (path.startsWith("reports/")) return c(ArtifactSemanticType.HUMAN_READABLE_REPORT,
                ProducingStage.PACKAGE);
        if (path.equals("policy-scenario-measurements.csv")) return c(
        ...
        if (path.startsWith("raw-data/bundles/")) {
            ...
            return switch (name) {
                case "run.csv" -> c(ArtifactSemanticType.RAW_RUN_METADATA,
                        ProducingStage.BENCHMARK_EVIDENCE);
                case "policies.csv" -> c(ArtifactSemanticType.RAW_POLICY_CATALOG,
                        ProducingStage.BENCHMARK_EVIDENCE);
                case "observations.csv" -> c(ArtifactSemanticType.RAW_OBSERVATIONS,
                        ProducingStage.BENCHMARK_EVIDENCE);
                case "COMPLETE" -> c(ArtifactSemanticType.COMPLETION_MARKER,
                        ProducingStage.BENCHMARK_EVIDENCE);
    ...
    private static List<String> sourceRuns(PackageSourceSet source, String path) {
        ...
        if (path.startsWith("model/") && source.model() != null) {
            ...
            return source.evidence().stream().filter(item ->
                    item.context().descriptor().closedLoopIteration() < iteration)
                    .map(item -> item.index().benchmarkRunId()).sorted().toList();
        }
        ...
        return source.evidence().stream().map(item -> item.index().benchmarkRunId())
                .sorted().toList();
    }
    ...
    private static ArtifactOrigin origin(PackageSourceSet source, List<String> runIds) {
        ...
        for (String runId : runIds) {
            EvidenceOrigin origin = source.evidence().stream().filter(item ->
                    item.index().benchmarkRunId().equals(runId)).findFirst().orElseThrow().origin();
        ...
        return nativeFound ? ArtifactOrigin.UPGRADED_RUN
    ...
    private static void cleanupOwnedStaging(Path outputRoot, String targetName,
            String packageId, Path current) throws IOException {
        try (var stream = Files.list(outputRoot)) {
            for (Path candidate : stream.filter(path -> path.getFileName().toString()
                    .startsWith("." + targetName + ".tmp-")).toList()) {
                ...
                if (Files.isSymbolicLink(candidate)
                        ...
                        .equals(packageId + "\n")) {
                    throw new PackageCollisionException(
                            "Unowned or ambiguous staging directory exists");
                }
