...
public record ClosedLoopCheckpoint(
    ...
    public ClosedLoopCheckpoint {
        requiredScenarios = java.util.Collections.unmodifiableSortedSet(new TreeSet<>(requiredScenarios));
        rotationCursors = java.util.Collections.unmodifiableSortedMap(new TreeMap<>(rotationCursors));
        evidence = evidence.stream()
                .sorted(java.util.Comparator.comparing(EvidenceIndexEntry::benchmarkRunId))
                .toList();
        carryForward = carryForward.stream()
                .sorted(java.util.Comparator.comparing(entry -> entry.policy().id()))
                .toList();
        pendingRuns = pendingRuns.stream()
                .sorted(java.util.Comparator.comparing(PendingBenchmarkRun::scenario))
                .toList();
        if (schemaVersion != 1
                || revision < 0
                || nextIteration < 0
        ...
        java.util.TreeSet<RotationGroup> expectedGroups = requiredScenarios.stream()
                .map(scenario -> new RotationGroup(scenario.environmentId(), scenario.availablePhysicalCoreCount()))
        ...
        for (var entry : rotationCursors.entrySet()) {
            long count = requiredScenarios.stream()
                    .filter(scenario ->
                            scenario.environmentId().equals(entry.getKey().environmentId())
                                    && scenario.availablePhysicalCoreCount()
                                            == entry.getKey().availablePhysicalCoreCount())
                    .count();
            if (entry.getValue() < 0 || entry.getValue() >= count) {
                throw new IllegalArgumentException("Invalid rotation cursor");
            }
        }
        if (evidence.stream().map(EvidenceIndexEntry::benchmarkRunId).distinct().count() != evidence.size()
                || pendingRuns.stream()
                                .map(PendingBenchmarkRun::benchmarkRunId)
                                .distinct()
                                .count()
                        != pendingRuns.size()) {
        ...
        java.util.Map<String, EvidenceIndexEntry> evidenceByRun = evidence.stream()
                .collect(java.util.stream.Collectors.toMap(
                        EvidenceIndexEntry::benchmarkRunId, java.util.function.Function.identity()));
        for (PendingBenchmarkRun pending : pendingRuns) {
            if (!requiredScenarios.contains(pending.scenario())
                    || pendingSchedule.isPresent() && !pending.schedule().equals(pendingSchedule.orElseThrow())
                    || pending.status() == PendingRunStatus.COMPLETE
                            && (!evidenceByRun.containsKey(pending.benchmarkRunId())
                                    || !evidenceByRun
                                            .get(pending.benchmarkRunId())
                                            .scenario()
        ...
        if (calibrated && (anchorSetId.isEmpty() || calibrationPlan.isEmpty() || latestMerge.isEmpty())) {
        ...
        boolean scheduleRequired = stage == CheckpointStage.SCHEDULE_READY
                || stage == CheckpointStage.BENCHMARKING
                || stage == CheckpointStage.READY_TO_MERGE;
        if (scheduleRequired != pendingSchedule.isPresent() || scheduleRequired && pendingRuns.isEmpty()) {
