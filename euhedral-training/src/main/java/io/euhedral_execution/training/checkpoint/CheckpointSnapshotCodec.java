...
public final class CheckpointSnapshotCodec {
    ...
    private static final List<String> SIDECARS = List.of("required-scenarios.csv",
            "rotation-cursors.csv", "evidence-index.csv", "carry-forward.csv",
            "carry-forward-scenarios.csv", "pending-runs.csv");
    private static final List<String> INVENTORY = List.of("state.csv",
            "required-scenarios.csv", "rotation-cursors.csv", "evidence-index.csv",
            "carry-forward.csv", "carry-forward-scenarios.csv", "pending-runs.csv",
            "COMPLETE");
    private static final List<String> STATE_HEADER = List.of("schema_version", "artifact_type",
            "training_run_id", "revision", "stage", "next_iteration", "sobol_cursor",
            "config_sha256", "required_scenarios_sha256", "rotation_cursors_sha256",
            "evidence_index_sha256", "carry_forward_sha256",
            "carry_forward_scenarios_sha256", "pending_runs_sha256", "anchor_set_id",
            "calibration_plan_path", "calibration_plan_sha256", "latest_merge_path",
            "latest_merge_sha256", "latest_model_path", "latest_model_sha256",
            "pending_schedule_path", "pending_schedule_sha256");
    ...
    public static Optional<LoadedCheckpoint> loadLatest(Path workspace,
        ...
        try (var stream = Files.list(checkpoints)) {
            complete = stream.filter(path -> path.getFileName().toString()
    ...
    private static void validatePrevious(Path workspace, ClosedLoopCheckpoint next)
            throws IOException {
        ...
        if (complete.isEmpty()) {
            if (next.revision() != 1 || next.stage() != CheckpointStage.BOOTSTRAP_PENDING
        ...
        if (next.sobolCursor() != previous.sobolCursor()
                && next.stage() != CheckpointStage.SCHEDULE_READY) {
    ...
    private static boolean validTransition(CheckpointStage from, CheckpointStage to) {
        return switch (from) {
            case BOOTSTRAP_PENDING -> to == CheckpointStage.BOOTSTRAP_PENDING
                    || to == CheckpointStage.READY_TO_TRAIN;
    ...
    private static String state(ClosedLoopCheckpoint checkpoint, Path directory)
            throws IOException {
        List<String> row = new ArrayList<>(List.of("1", ARTIFACT_TYPE,
                checkpoint.trainingRunId(), Integer.toString(checkpoint.revision()),
                checkpoint.stage().name(), Integer.toString(checkpoint.nextIteration()),
                Long.toString(checkpoint.sobolCursor()), checkpoint.configSha256()));
    ...
    private static String carry(ClosedLoopCheckpoint checkpoint) {
        List<String> header = new ArrayList<>(List.of("schema_version", "policy_id",
                ...
                "pessimistic_missing_quality", "maximum_missing_epistemic_stddev",
    ...
    private static String carryScenarios(ClosedLoopCheckpoint checkpoint) {
        StringBuilder out = new StringBuilder(Phase3Csv.row(List.of("schema_version",
                ...
                "ordinal_entropy", "top_decile_probability", "epistemic_stddev",
                "disagreement_range")));
        checkpoint.carryForward().stream().sorted(Comparator.comparing(entry ->
                ...
                (scenario, state) -> {
                    ...
                    out.append(Phase3Csv.row(List.of("1", entry.policy().id().canonical(),
                            ...
                            state.lastAttemptIteration().isPresent()
                                    ? Integer.toString(state.lastAttemptIteration().getAsInt()) : "",
    ...
    private static String pending(ClosedLoopCheckpoint checkpoint) {
        ...
        checkpoint.pendingRuns().stream().sorted(Comparator.comparing(
                ...
                row.evidenceRelativePath(), row.status().name()))));
        return out.toString();
    }
    ...
    private static ClosedLoopCheckpoint read(Path workspace, Path directory) {
        try {
            ...
            for (int i = 0; i < SIDECARS.size(); i++) {
                if (!row.get(8 + i).equals(fileSha256(
                        directory.resolve(SIDECARS.get(i))))) {
                    throw new IllegalArgumentException("Checkpoint sidecar checksum mismatch");
                }
            }
            TreeSet<SourceScenario> scenarios = readScenarios(
                    directory.resolve("required-scenarios.csv"));
            ...
            if (directory.getFileName().toString().matches("checkpoint-[0-9]{8}")
                    && revision != revision(directory)) {
                throw new IllegalArgumentException("Checkpoint directory revision mismatch");
            }
            return new ClosedLoopCheckpoint(1, row.get(2), revision,
                    CheckpointStage.valueOf(row.get(4)), integer(row.get(5)),
                    Long.parseLong(row.get(6)), row.get(7), scenarios, cursors, evidence, carry,
                    row.get(14).isEmpty() ? Optional.empty() : Optional.of(row.get(14)),
                    calibration, merge, model, schedule, pending);
    ...
    private static TreeSet<SourceScenario> readScenarios(Path file) throws IOException {
        ...
        for (int i = 1; i < rows.size(); i++) {
            ...
            version(row.get(0));
            SourceScenario scenario = SourceScenario.of(row.get(2), integer(row.get(3)),
                    integer(row.get(4)));
    ...
    private static TreeMap<RotationGroup, Integer> readCursors(Path file) throws IOException {
        ...
        for (int i = 1; i < rows.size(); i++) {
            ...
            if (previous != null && group.compareTo(previous) <= 0
                    || result.put(group, integer(row.get(3))) != null) {
    ...
    private static List<CarryForwardEntry> readCarry(Path summaryFile, Path scenarioFile,
        ...
        for (int i = 1; i < summaries.size(); i++) {
            ...
            if (!policy.id().equals(PolicyId.parse(row.get(1)))
                    ...
                    || metadata.put(policy.id(), new CarrySummary(policy, integer(row.get(2)),
                    integer(row.get(3)), integer(row.get(4)), integer(row.get(5)),
                    finite(row.get(6)), finite(row.get(7)), finite(row.get(8)))) != null) {
        ...
        requireHeader(rows, List.of("schema_version", "policy_id", "scenario_id",
                ...
                "quality_interval_low", "quality_interval_high", "ordinal_entropy",
                "top_decile_probability", "epistemic_stddev", "disagreement_range"));
        TreeMap<PolicyId, SortedMap<SourceScenario, CarryScenarioState>> grids =
                new TreeMap<>();
        ...
        for (int i = 1; i < rows.size(); i++) {
            ...
            if (previousGridPolicy != null
                    ...
                    || policy.equals(previousGridPolicy)
                    && scenario.compareTo(previousGridScenario) <= 0)) {
                throw new IllegalArgumentException("Carry scenarios are not sorted");
            }
            ScenarioPrediction prediction = new ScenarioPrediction(scenario, finite(row.get(7)),
                    finite(row.get(8)), finite(row.get(9)), finite(row.get(10)),
                    finite(row.get(11)), finite(row.get(12)), finite(row.get(13)),
                    finite(row.get(14)));
            CarryScenarioState state = new CarryScenarioState(scenario,
                    CoverageState.valueOf(row.get(3)), integer(row.get(4)),
                    row.get(5).isEmpty() ? OptionalInt.empty()
                            : OptionalInt.of(integer(row.get(5))),
                    integer(row.get(6)), prediction);
            if (grids.computeIfAbsent(policy, ignored -> new TreeMap<>())
                    .put(scenario, state) != null) {
        ...
        for (var entry : metadata.entrySet()) {
            ...
            CarryForwardEntry carry = new CarryForwardEntry(summary.policy(),
                    summary.firstSeen(), summary.lastUpdated(), grid);
    ...
    private static List<PendingBenchmarkRun> readPending(Path file) throws IOException {
        ...
        for (int i = 1; i < rows.size(); i++) {
            ...
            result.add(new PendingBenchmarkRun(integer(row.get(1)),
                    RunKind.valueOf(row.get(2)), scenario, row.get(4), row.get(5),
                    new ArtifactReference(row.get(6), row.get(7)), row.get(8),
    ...
    private static void validateInventory(Path directory) throws IOException {
        ...
        try (var stream = Files.list(directory)) {
            if (!stream.map(path -> path.getFileName().toString()).sorted().toList()
    ...
    private static void validateArtifacts(Path workspace,
            List<Optional<ArtifactReference>> artifacts) throws IOException {
    ...
    private static void validateArtifact(Path workspace, ArtifactReference reference)
            throws IOException {
        Path target = workspace.resolve(reference.relativePath()).normalize();
        if (!target.startsWith(workspace)
                || !ArtifactFingerprint.sha256(target).equals(reference.sha256())) {
    ...
    private static Optional<ArtifactReference> artifact(String path, String hash) {
        ...
        return path.isEmpty() ? Optional.empty()
                : Optional.of(new ArtifactReference(path, hash));
    }

    private static void appendArtifact(List<String> row,
    ...
    private static int observedCount(CarryForwardEntry entry) {
        return Math.toIntExact(entry.scenarios().values().stream().filter(state ->
                state.coverage() != CoverageState.MISSING).count());
    }
    ...
    private record CarrySummary(PolicyVector policy, int firstSeen, int lastUpdated,
            int valid, int observed, double pessimistic, double maxEpistemic,
