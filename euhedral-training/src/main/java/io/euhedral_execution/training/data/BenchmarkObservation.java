...
public record BenchmarkObservation(ObservationKey key, BenchmarkRunDescriptor run,
                                   ScheduledPolicy scheduledPolicy, ObservationStatus status,
                                   MeasurementEncoding measurementEncoding, Instant startedAt,
                                   Instant endedAt, OptionalLong elapsedNanos,
                                   OptionalLong completedFrames,
                                   OptionalDouble throughputFramesPerSecond, String failureCode) {
    ...
    public BenchmarkObservation {
        ...
        if (failureCode == null || (!failureCode.isEmpty() && !failureCode.matches(
                "[A-Z][A-Z0-9_]{0,63}"))) {
        ...
        if (!key.benchmarkRunId().equals(run.benchmarkRunId()) || !key.scenario()
                .equals(run.scenario()) || !key.policyId().equals(scheduledPolicy.policy().id())
                || key.repetitionNumber() > run.parameters().expectedRepetitions()
                || startedAt.isBefore(run.startedAt()) || endedAt.isBefore(startedAt)) {
        ...
        if (measurementEncoding == MeasurementEncoding.DIRECT_THROUGHPUT && (
                elapsedNanos.isPresent() != completedFrames.isPresent()
                        || elapsedNanos.isPresent() && ((elapsedNanos.getAsLong() > 0)
                        != throughputFramesPerSecond.isPresent()))) {
        ...
        if (measurementEncoding == MeasurementEncoding.COUNTER_DERIVED) {
            if (elapsedNanos.isEmpty() || completedFrames.isEmpty() || elapsedNanos.getAsLong() < 0
            ...
            if (elapsedNanos.getAsLong() > 0) {
                double expected =
                        completedFrames.getAsLong() * 1_000_000_000.0 / elapsedNanos.getAsLong();
        ...
        if (status == ObservationStatus.SUCCESS && (throughputFramesPerSecond.isEmpty()
                || !Double.isFinite(throughputFramesPerSecond.getAsDouble())
                || throughputFramesPerSecond.getAsDouble() <= 0 || !failureCode.isEmpty())) {
        ...
        if (status == ObservationStatus.SKIPPED && (!startedAt.equals(endedAt)
                || elapsedNanos.isEmpty() || completedFrames.isEmpty()
                || elapsedNanos.getAsLong() != 0 || completedFrames.getAsLong() != 0
                || throughputFramesPerSecond.isPresent() || failureCode.isEmpty())) {
            throw new IllegalArgumentException("Invalid skipped observation");
        }
        if ((status == ObservationStatus.TIMEOUT || status == ObservationStatus.FAILED)
                && failureCode.isEmpty()) {
