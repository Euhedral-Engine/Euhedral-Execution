...
public record BenchmarkObservation(
    ...
    public BenchmarkObservation {
        ...
        if (measurementEncoding == MeasurementEncoding.COUNTER_DERIVED) {
            if (elapsedNanos.isEmpty() || completedFrames.isEmpty()
            ...
            if (elapsedNanos.getAsLong() > 0) {
                double expected = completedFrames.getAsLong() * 1_000_000_000.0
        ...
        if (status == ObservationStatus.SKIPPED
                && (!startedAt.equals(endedAt) || elapsedNanos.isEmpty()
                || completedFrames.isEmpty() || elapsedNanos.getAsLong() != 0
                || completedFrames.getAsLong() != 0 || throughputFramesPerSecond.isPresent()
                || failureCode.isEmpty())) {
