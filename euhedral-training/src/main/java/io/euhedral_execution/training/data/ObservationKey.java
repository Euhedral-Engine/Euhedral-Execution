...
public record ObservationKey(String benchmarkRunId, SourceScenario scenario, PolicyId policyId, int repetitionNumber)
        implements Comparable<ObservationKey> {
    ...
    public ObservationKey {
        ...
        if (benchmarkRunId == null
                ...
                || repetitionNumber < 1
                || repetitionNumber > 999_999) {
            throw new IllegalArgumentException("Invalid observation key");
        }
    }

    public String canonical() {
        return "ob1/" + benchmarkRunId + "/" + scenario.canonical() + "/" + policyId.canonical() + "/rep-"
                + String.format("%06d", repetitionNumber);
    }
