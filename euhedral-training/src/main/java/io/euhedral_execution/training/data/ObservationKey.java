...
public record ObservationKey(String benchmarkRunId, SourceScenario scenario, PolicyId policyId,
                             int repetitionNumber) implements Comparable<ObservationKey> {

    public ObservationKey {
        Objects.requireNonNull(scenario);
        Objects.requireNonNull(policyId);
        if (benchmarkRunId == null || !benchmarkRunId.matches("[a-z0-9][a-z0-9._-]{0,95}")
    ...
    public String canonical() {
        return "ob1/" + benchmarkRunId + "/" + scenario.canonical() + "/" + policyId.canonical()
                + "/rep-" + String.format("%06d", repetitionNumber);
    }
