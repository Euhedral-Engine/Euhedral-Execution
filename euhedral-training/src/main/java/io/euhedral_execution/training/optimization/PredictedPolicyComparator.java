...
public final class PredictedPolicyComparator {
    public static final Comparator<PredictedPolicySummary> BEST_FIRST = (left, right) -> {
        ...
        if (result == 0) result = descending(left.predictedGeometricMeanQuality(),
                right.predictedGeometricMeanQuality());
        if (result == 0) result = Double.compare(left.predictedQualityMad(),
                right.predictedQualityMad());
        if (result == 0) result = Double.compare(left.maximumEpistemicStdDev(),
                right.maximumEpistemicStdDev());
        if (result == 0) result = Double.compare(left.maximumDisagreementRange(),
                right.maximumDisagreementRange());
        if (result == 0) result = Double.compare(left.meanOrdinalStdDev(),
                right.meanOrdinalStdDev());
    ...
    public static final Comparator<PredictedPolicySummary> AUDIT_FIRST = (left, right) -> {
        ...
        if (result == 0) result = descending(left.maximumDisagreementRange(),
                right.maximumDisagreementRange());
