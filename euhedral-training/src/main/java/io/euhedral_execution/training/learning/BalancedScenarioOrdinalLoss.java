...
final class BalancedScenarioOrdinalLoss extends Loss {
    private final NDArray positiveWeights;
    private final NDArray negativeWeights;
    private final float labelSmoothing;

    BalancedScenarioOrdinalLoss(NDManager manager, ClassBalance balance, float labelSmoothing) {
    ...
    static ClassBalance fit(ScenarioLearningMatrix matrix) {
    ...
    static double weightedBce(float[] logits, ScenarioLearningMatrix matrix,
    ...
    private static double compensated(float[] values) {
        CompensatedSum sum = new CompensatedSum();
        for (float value : values) sum.add(value);
        return sum.value();
    }

    record ClassBalance(float[] positiveWeights, float[] negativeWeights, float[] positiveRates) {
        ClassBalance {
        ...
        @Override public float[] positiveWeights() { return positiveWeights.clone(); }
        @Override public float[] negativeWeights() { return negativeWeights.clone(); }
        @Override public float[] positiveRates() { return positiveRates.clone(); }
    }
