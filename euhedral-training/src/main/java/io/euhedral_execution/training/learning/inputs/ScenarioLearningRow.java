...
public record ScenarioLearningRow(
    ...
    public ScenarioLearningRow {
        ...
        if (!rate(quality)
                || !positive(throughputMedian)
                || !positive(bootstrapMedianCiLow)
                || !positive(bootstrapMedianCiHigh)
                || bootstrapMedianCiLow > bootstrapMedianCiHigh
                || acceptedRunCount <= 0
                || !nonnegative(medianWithinRunRelativeIqr)
                || !rate(meanNonSuccessRate)) {
            throw new IllegalArgumentException("Invalid scenario learning row");
        }
    }

    private static boolean rate(double x) {
    ...
    private static boolean nonnegative(double x) {
        return Double.isFinite(x) && x >= 0;
    }

    @Override
    public int compareTo(ScenarioLearningRow other) {
