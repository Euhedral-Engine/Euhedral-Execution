...
public record ScenarioTrainingConfig(
    ...
    public ScenarioTrainingConfig {
        ...
        if (!device.equals("auto") && !device.equals("cpu") && !device.matches("gpu[0-9]+")
                ...
                || losoEvaluationMembers > ensembleMembers
                || ablationMembers < 3
                || ablationMembers > ensembleMembers
                || ablationMembers % 2 == 0
                || maxEpochs <= 0
                || patience <= 0
                || batchSize < 0
                || !Float.isFinite(learningRate)
                || learningRate <= 0
                || !Float.isFinite(weightDecay)
                || weightDecay < 0
                ...
                || minimumTestPolicyGroups < 1
                || minimumTrainRowsPerScenario < 1
                || minimumValidationRowsPerScenario < 1
                || minimumTestRowsPerScenario < 1) {
    ...
    public static ScenarioTrainingConfig forTest(long splitSeed, long modelSeed, FeatureSelectionMode mode) {
        return new ScenarioTrainingConfig(
                splitSeed,
                modelSeed,
                "cpu",
    ...
    public boolean isEffectiveVersionOf(ScenarioTrainingConfig requested) {
        ...
        boolean compatibleBatch = batchSize == requested.batchSize
                || batchSize > 0 && (requested.batchSize == 0 || batchSize <= requested.batchSize);
        return compatibleBatch
                ...
                && Float.floatToRawIntBits(learningRate) == Float.floatToRawIntBits(requested.learningRate)
                && Float.floatToRawIntBits(weightDecay) == Float.floatToRawIntBits(requested.weightDecay)
                && Float.floatToRawIntBits(labelSmoothing) == Float.floatToRawIntBits(requested.labelSmoothing)
