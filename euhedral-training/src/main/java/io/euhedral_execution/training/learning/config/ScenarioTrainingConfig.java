...
public record ScenarioTrainingConfig(long splitSeed, long modelSeed, String device,
                                     ...
                                     int ablationMembers, int maxEpochs, int patience,
                                     int batchSize, float learningRate, float weightDecay,
                                     float labelSmoothing, int minimumTrainPolicyGroups,
                                     int minimumValidationPolicyGroups, int minimumTestPolicyGroups,
    ...
    public static ScenarioTrainingConfig defaults() {
        return new ScenarioTrainingConfig(0x243f6a8885a308d3L, 0x13198a2e03707344L, "auto", 5, 1, 3,
                250, 20, 0, .001f, .0001f, .02f, 40, 10, 10, 30, 5, 5, false,
    ...
    public static ScenarioTrainingConfig forTest(long splitSeed, long modelSeed,
        ...
        return new ScenarioTrainingConfig(splitSeed, modelSeed, "cpu", 3, 1, 3, 5, 2, 16, 0.001f,
                0.0001f, 0.02f, 1, 2, 1, 1, 1, 1, false, mode, EvaluationThresholds.defaults());
    }
    ...
    public ScenarioTrainingConfig {
        ...
        if (!device.equals("auto") && !device.equals("cpu") && !device.matches("gpu[0-9]+")
                || ensembleMembers > 9 || ensembleMembers % 2 == 0 || losoEvaluationMembers < 1
                || losoEvaluationMembers > ensembleMembers || ablationMembers < 3
                || ablationMembers > ensembleMembers || ablationMembers % 2 == 0 || maxEpochs <= 0
                || patience <= 0 || batchSize < 0 || !Float.isFinite(learningRate)
                || learningRate <= 0 || !Float.isFinite(weightDecay) || weightDecay < 0
                || !Float.isFinite(labelSmoothing) || labelSmoothing < 0 || labelSmoothing >= .5
