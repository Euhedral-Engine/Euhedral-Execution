...
public record LosoEvaluationMetrics(ScenarioEvaluationMetrics metrics, double heldOutRatio,
        boolean ratioSeenInFit, int fittingScenarioCount, int fittingDistinctRatioCount) {
    public LosoEvaluationMetrics {
        Objects.requireNonNull(metrics);
        if (!Double.isFinite(heldOutRatio) || heldOutRatio <= 0
                || Double.compare(heldOutRatio,
                metrics.scenario().ratio().asDouble()) != 0
                || fittingScenarioCount < 0 || fittingDistinctRatioCount < 0
