...
public record ScenarioModelMetadata(int schemaVersion, String objectiveVersion,
        ...
        List<String> ordinalThresholdBits, String architecture, String memberModelName,
        List<MemberMetadata> members, String splitAlgorithm, long splitSeed, long modelSeed,
        String datasetFingerprintSha256, boolean includeWeakCalibrationRows,
        SortedSet<SourceScenario> requiredScenarios, SortedSet<SourceScenario> trainingScenarios,
        PartitionCounts partitionCounts, ScenarioTrainingConfig trainingConfig,
        FeatureSelectionDecision featureSelection, EvaluationSummaryMetadata evaluationSummary,
    ...
    public static final int PHASE1_SCHEMA_VERSION = 1;
        ...
        for (var counts : partitionCounts.scenarioRowCounts().values()) {
            if (!counts.keySet().equals(requiredScenarios)) {
                throw new IllegalArgumentException(
                        "Partition scenarios disagree with required scenarios");
            }
        }
