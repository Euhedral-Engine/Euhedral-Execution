...
public record MetadataProbe(PolicyId policyId, SourceScenario scenario,
    ...
    public static MetadataProbe from(PolicyId policyId, ScenarioPrediction prediction,
            String device) {
        return new MetadataProbe(policyId, prediction.scenario(),
                List.of(bits(prediction.predictedQuality()), bits(prediction.ordinalStdDev()),
                        bits(prediction.qualityIntervalLow()),
                        bits(prediction.qualityIntervalHigh()), bits(prediction.ordinalEntropy()),
                        bits(prediction.topDecileProbability()), bits(prediction.epistemicStdDev()),
                        bits(prediction.disagreementRange())), device);
    }
    ...
    public MetadataProbe {
        ...
        if (predictionRawBits.size() != 8 || predictionRawBits.stream()
                .anyMatch(bits -> !bits.matches("[0-9a-f]{16}")) || !producingDevice.matches(
                "(?:cpu|gpu[0-9]+)")) {
        ...
        double[] prediction = predictionRawBits.stream()
                ...
                .toArray();
        new ScenarioPrediction(scenario, prediction[0], prediction[1], prediction[2], prediction[3],
