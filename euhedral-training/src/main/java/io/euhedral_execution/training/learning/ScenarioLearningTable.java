...
public record ScenarioLearningTable(List<ScenarioLearningRow> rows,
        SortedMap<PolicyId, PolicyVector> policies, SortedSet<SourceScenario> requiredScenarios,
    ...
    public ScenarioLearningTable {
        ...
        if (audit.policyCount() != policies.size()
                ...
                || audit.phase1RowCount() != audit.includedStrongRowCount()
        ...
        for (int i = 0; i < rows.size(); i++) {
            ScenarioLearningRow row = rows.get(i);
