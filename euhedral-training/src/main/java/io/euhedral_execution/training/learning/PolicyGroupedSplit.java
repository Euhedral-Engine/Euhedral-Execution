...
public record PolicyGroupedSplit(SortedMap<PolicyId, LearningPartition> policyPartitions,
        List<ScenarioLearningRow> trainingRows, List<ScenarioLearningRow> validationRows,
        List<ScenarioLearningRow> testRows, SortedSet<PolicyId> ablationEarlyStopPolicies,
        SortedSet<PolicyId> ablationScorePolicies, List<ScenarioLearningRow> ablationEarlyStopRows,
    ...
    public PolicyGroupedSplit {
        ...
        trainingRows = List.copyOf(trainingRows); validationRows = List.copyOf(validationRows);
        testRows = List.copyOf(testRows);
        ablationEarlyStopPolicies = Collections.unmodifiableSortedSet(new TreeSet<>(ablationEarlyStopPolicies));
        ablationScorePolicies = Collections.unmodifiableSortedSet(new TreeSet<>(ablationScorePolicies));
