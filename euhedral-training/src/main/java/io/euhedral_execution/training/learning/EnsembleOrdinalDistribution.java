...
public record EnsembleOrdinalDistribution(double[] meanBinMasses, double predictedQuality,
        double ordinalStdDev, double qualityIntervalLow, double qualityIntervalHigh,
        double ordinalEntropy, double topDecileProbability, double epistemicStdDev,
        double disagreementRange) {
    public EnsembleOrdinalDistribution {
        if (meanBinMasses == null) throw new NullPointerException();
        ...
        for (double mass : meanBinMasses) {
            if (!nonnegative(mass)) throw new IllegalArgumentException("Invalid bin mass");
            total += mass;
        }
    ...
    @Override public double[] meanBinMasses() { return meanBinMasses.clone(); }
