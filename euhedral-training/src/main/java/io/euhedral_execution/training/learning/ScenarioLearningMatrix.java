...
public record ScenarioLearningMatrix(int rows, int featureWidth, float[] features,
        float[] ordinalLabels, float[] rowWeights, double[] qualities, PolicyId[] policyIds,
        SourceScenario[] scenarios) {
    public ScenarioLearningMatrix {
        features = features.clone(); ordinalLabels = ordinalLabels.clone();
        rowWeights = rowWeights.clone(); qualities = qualities.clone();
        policyIds = policyIds.clone(); scenarios = scenarios.clone();
        if (rows <= 0 || featureWidth <= 0
                || features.length != (long) rows * featureWidth
                || ordinalLabels.length != (long) rows * 9 || rowWeights.length != rows
                || qualities.length != rows || policyIds.length != rows || scenarios.length != rows)
    ...
    @Override public float[] features() { return features.clone(); }
    @Override public float[] ordinalLabels() { return ordinalLabels.clone(); }
    @Override public float[] rowWeights() { return rowWeights.clone(); }
    @Override public double[] qualities() { return qualities.clone(); }
    @Override public PolicyId[] policyIds() { return policyIds.clone(); }
    @Override public SourceScenario[] scenarios() { return scenarios.clone(); }
}
