...
public final class PolicyGroupedSplitter {
    ...
    public static PolicyGroupedSplit split(ScenarioLearningTable table, long seed, ScenarioTrainingConfig config) {
        ...
        ArrayList<ScenarioLearningRow> train = new ArrayList<>(),
                validation = new ArrayList<>(),
                test = new ArrayList<>(),
                earlyRows = new ArrayList<>(),
        ...
        check(
                ...
                config.minimumTrainPolicyGroups(),
        ...
        check(
                ...
                test,
                table.requiredScenarios(),
                config.minimumTestPolicyGroups(),
        ...
        if (requireVariation) {
            ...
            check("ablation early stop", earlyRows, table.requiredScenarios(), halfGroups, halfRows, true, false);
            check("ablation score", scoreRows, table.requiredScenarios(), halfGroups, halfRows, true, true);
        }
        return new PolicyGroupedSplit(assignments, train, validation, test, early, score, earlyRows, scoreRows);
    }
    ...
    public static List<ScenarioLearningRow> withoutScenario(List<ScenarioLearningRow> rows, SourceScenario heldOut) {
        return rows.stream().filter(r -> !r.scenario().equals(heldOut)).toList();
    }

    public static List<ScenarioLearningRow> onlyScenario(List<ScenarioLearningRow> rows, SourceScenario heldOut) {
        return rows.stream().filter(r -> r.scenario().equals(heldOut)).toList();
    }

    public static List<ScenarioLearningRow> withoutEnvironment(List<ScenarioLearningRow> rows, String environment) {
        return rows.stream()
                .filter(r -> !r.scenario().environmentId().equals(environment))
                .toList();
    }
    ...
    private static void check(
            ...
            int groups,
            int minimumRows,
            boolean requireScenarioCoverage,
        ...
        for (var scenario : scenarios) {
            ...
            if (targetChecks
                    && (selected.stream()
                                            .map(ScenarioLearningRow::quality)
                                            .distinct()
                                            .count()
                                    < 2
