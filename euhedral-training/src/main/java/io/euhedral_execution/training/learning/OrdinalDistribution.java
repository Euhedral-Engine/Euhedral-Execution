...
public record OrdinalDistribution(double[] cumulativeProbabilities, double[] binMasses,
        double meanQuality, double variance, double entropy, double topDecileProbability) {
    public OrdinalDistribution {
        if (cumulativeProbabilities == null || binMasses == null) {
            throw new NullPointerException();
        }
        cumulativeProbabilities = cumulativeProbabilities.clone(); binMasses = binMasses.clone();
        ...
        for (double value : binMasses) {
            if (!nonnegative(value)) throw new IllegalArgumentException("Invalid bin mass");
            mass += value;
        }
    ...
    @Override public double[] cumulativeProbabilities() { return cumulativeProbabilities.clone(); }
    @Override public double[] binMasses() { return binMasses.clone(); }
