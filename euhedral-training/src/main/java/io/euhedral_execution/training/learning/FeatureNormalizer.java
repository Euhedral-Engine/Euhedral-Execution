...
public record FeatureNormalizer(String featureSchemaId, List<String> featureNames,
    ...
    public FeatureNormalizer {
        ...
        featureNames = List.copyOf(featureNames); means = means.clone(); scales = scales.clone();
    ...
    @Override public double[] means() { return means.clone(); }
    @Override public double[] scales() { return scales.clone(); }
    @Override public boolean[] constantFeatures() { return constantFeatures.clone(); }

    public static FeatureNormalizer fit(List<ScenarioLearningRow> rows, ScenarioFeatureSet set) {
        Objects.requireNonNull(rows);
        Objects.requireNonNull(set);
        if (rows.isEmpty()) throw new IllegalArgumentException("Training rows are empty");
        ...
        for (int f = 0; f < set.width(); f++) {
            double[] values;
            if (f < 28) {
                values = new double[policies.size()]; int i = 0;
                for (PolicyVector p : policies.values()) values[i++] = p.weight(f);
            } else {
                values = new double[scenarios.size()]; int i = 0;
                for (SourceScenario s : scenarios) values[i++] = raw(s, f);
            }
            mean[f] = sum(values) / values.length;
            double[] deviations = new double[values.length];
            for (int i = 0; i < values.length; i++) {
                double d = values[i] - mean[f]; deviations[i] = d * d;
            }
            double variance = sum(deviations) / values.length;
            if (!Double.isFinite(variance)) throw new IllegalArgumentException("Non-finite variance");
            if (variance < 0) {
    ...
    public void encode(PolicyVector policy, SourceScenario scenario, float[] out, int offset) {
        ...
        if (offset < 0 || offset + means.length > out.length) throw new IndexOutOfBoundsException();
        for (int i = 0; i < means.length; i++) {
            double value = i < 28 ? policy.weight(i) : raw(scenario, i);
            float encoded = (float) ((value - means[i]) / scales[i]);
            if (!Float.isFinite(encoded)) throw new IllegalArgumentException(
