...
public final class ScenarioOrdinalTargets {
    ...
    public static double threshold(int k) {
        if (k < 0 || k >= 9) throw new IndexOutOfBoundsException();
        return (k + 1) / 10.0;
    }
    public static void encode(double quality, float[] out, int offset) {
        ...
        for (int k = 0; k < 9; k++) out[offset + k] = quality >= threshold(k) ? 1 : 0;
    }
    public static OrdinalDistribution decode(double[] logits) {
        Objects.requireNonNull(logits);
        if (logits.length != 9) throw new IllegalArgumentException("Expected nine logits");
        double[] p = new double[9];
        for (int i = 0; i < 9; i++) {
            double x = logits[i];
            if (!Double.isFinite(x)) throw new IllegalArgumentException("Non-finite logit");
            if (x >= 0) {
        ...
        pav(p);
        double[] mass = new double[10]; mass[0] = 1 - p[0];
        for (int b = 1; b < 9; b++) mass[b] = p[b - 1] - p[b];
        mass[9] = p[8];
        CompensatedSum total = new CompensatedSum();
        for (int b = 0; b < 10; b++) {
            if (mass[b] < -1e-15) throw new IllegalArgumentException("Negative ordinal mass");
            if (mass[b] < 0) mass[b] = 0;
            total.add(mass[b]);
        }
        ...
        for (int b = 0; b < 10; b++) {
            ...
            if (mass[b] > 0) entropySum.add(-mass[b] * StrictMath.log(mass[b]));
        }
    ...
    public static EnsembleOrdinalDistribution combine(List<OrdinalDistribution> members) {
        ...
        if (members.isEmpty()) throw new IllegalArgumentException("No members");
    ...
    static EnsembleOrdinalDistribution combineAggregated(double[] meanMasses,
        ...
        for (double value : memberMeans) {
            if (!finiteRate(value)) throw new IllegalArgumentException("Invalid member mean");
    ...
    static EnsembleOrdinalDistribution combineAggregatedUncertainty(double[] meanMasses,
        ...
        for (int b = 0; b < 10; b++) {
            ...
            meanSum.add(mass[b] * center(b));
            if (mass[b] > 0) entropy.add(-mass[b] * StrictMath.log(mass[b]));
        }
    ...
    private static void pav(double[] p) {
        double[] means = new double[9]; int[] starts = new int[9], sizes = new int[9]; int blocks = 0;
        for (int i = 0; i < 9; i++) {
            means[blocks] = p[i]; starts[blocks] = i; sizes[blocks++] = 1;
            while (blocks > 1 && Double.compare(means[blocks - 2], means[blocks - 1]) < 0) {
                ...
                sizes[blocks - 2] = n; blocks--;
            }
        }
        for (int b = 0; b < blocks; b++)
            for (int i = starts[b]; i < starts[b] + sizes[b]; i++) p[i] = means[b];
    }
    private static double quantile(double[] mass, double q) {
        CompensatedSum cumulative = new CompensatedSum();
        for (int b = 0; b < 10; b++) {
            cumulative.add(mass[b]);
            if (cumulative.value() >= q) return center(b);
        }
        return .95;
    }
    private static double center(int b) { return .05 + .10 * b; }
    private static double square(double x) { return x * x; }
    private static boolean finiteRate(double x) { return Double.isFinite(x) && x >= 0 && x <= 1; }

    private static double nonnegativeVariance(double value) {
        if (!Double.isFinite(value)) throw new IllegalArgumentException("Non-finite variance");
        if (value < 0 && value >= -1.0e-15) return 0;
        if (value < 0) throw new IllegalArgumentException("Negative variance");
        return value;
    }
