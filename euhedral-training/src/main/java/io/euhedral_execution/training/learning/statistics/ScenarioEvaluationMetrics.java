...
public record ScenarioEvaluationMetrics(
    ...
    public ScenarioEvaluationMetrics {
        ...
        if (policyCount < 1
                || policyCount > rowCount
                || actualTopDecileCount < 0
                || actualTopDecileCount > rowCount
                || selectedCount != StrictMath.max(1, (int) StrictMath.ceil(0.10 * rowCount))) {
            throw new IllegalArgumentException();
        }
        for (double x : new double[] {
            mae, rmse, meanBias, meanIntervalWidth, intervalCoverage95, meanEpistemicStdDev, meanDisagreementRange
        }) {
        ...
        if (mae < 0
                ...
                || intervalCoverage95 > 1
                || meanEpistemicStdDev < 0
                || meanEpistemicStdDev > 1
                || meanDisagreementRange < 0
                || meanDisagreementRange > 1
