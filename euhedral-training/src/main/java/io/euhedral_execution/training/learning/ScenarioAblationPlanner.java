...
final class ScenarioAblationPlanner {
    ...
    static Decision decide(
        ...
        addComparedMetrics(metrics, "VALIDATION_CONTEXT_LOSO", policyContextFolds, ratioContextFolds, false);
        boolean contextMetrics = sameFoldIds(policyContextFolds, ratioContextFolds)
                && policy.ok()
                && ratio.ok()
                && policy.mae().isPresent()
                && ratio.mae().isPresent()
                && policy.spearman().isPresent()
                && ratio.spearman().isPresent();
        double contextMaeDelta =
                contextMetrics ? ratio.mae().getAsDouble() - policy.mae().getAsDouble() : Double.NaN;
        ...
        boolean useful = contextMetrics
                && (ratio.mae().getAsDouble() <= policy.mae().getAsDouble() - thresholds.minimumContextMaeImprovement()
                        || ratio.spearman().getAsDouble()
                                >= policy.spearman().getAsDouble() + thresholds.minimumContextSpearmanImprovement());
        boolean contextPassed = useful
                && ratio.mae().getAsDouble() <= policy.mae().getAsDouble() + thresholds.maximumContextMaeRegression()
                && ratio.spearman().getAsDouble()
                        >= policy.spearman().getAsDouble() - thresholds.maximumContextSpearmanRegression();
        metrics.add(aggregateMetric(
                ...
                ratio,
                contextMaeDelta,
                contextSpearmanDelta,
                contextPassed,
                contextMetrics ? contextPassed ? "PASS" : "FAIL" : "NOT_EVALUABLE",
                contextMetrics
                        ? contextPassed ? "CONTEXT_VALIDATED" : "CONTEXT_GATE_FAILED"
                        : "CONTEXT_METRIC_MISSING"));

        if (mode == FeatureSelectionMode.RATIO_ONLY) {
            return new Decision(
                    new FeatureSelectionDecision(mode, ScenarioFeatureSet.RATIO_ONLY, metrics, "RATIO_ONLY_REQUESTED"),
                    contextPassed,
                    true);
        }
        if (environmentCount < 2) {
            ...
            return new Decision(
                    new FeatureSelectionDecision(mode, selected, metrics, "INSUFFICIENT_ENVIRONMENTS"),
                    contextPassed,
                    countsPassed);
        }
        Aggregate ratioEnvironment = aggregate(ratioEnvironmentFolds);
        Aggregate countsEnvironment = aggregate(countEnvironmentFolds);
        addComparedMetrics(metrics, "VALIDATION_COUNTS_LOEO", ratioEnvironmentFolds, countEnvironmentFolds, false);
        boolean countMetrics = ratioEnvironmentFolds.size() == environmentCount
                && sameFoldIds(ratioEnvironmentFolds, countEnvironmentFolds)
                && ratioEnvironment.ok()
                && countsEnvironment.ok()
                && ratioEnvironment.mae().isPresent()
                && countsEnvironment.mae().isPresent()
        ...
        double countMaeDelta = countMetrics
                ? countsEnvironment.mae().getAsDouble() - ratioEnvironment.mae().getAsDouble()
                : Double.NaN;
        ...
        boolean countsPassed = countMetrics
                ...
                && countsEnvironment.spearman().getAsDouble()
                        >= ratioEnvironment.spearman().getAsDouble() - thresholds.maximumCountsSpearmanRegression()
                && countsEnvironment.worstMae().getAsDouble()
                        <= ratioEnvironment.worstMae().getAsDouble()
                                + thresholds.maximumCountsWorstEnvironmentMaeRegression();
        metrics.add(aggregateMetric(
                "VALIDATION_COUNTS_GATE",
                "all",
                ScenarioFeatureSet.RATIO_AND_COUNTS,
                ScenarioFeatureSet.RATIO_ONLY,
                "all",
                ...
                countMetrics ? countsPassed ? "COUNTS_VALIDATED" : "COUNTS_GATE_FAILED" : "COUNTS_METRIC_MISSING"));
        ScenarioFeatureSet selected = countsPassed || mode == FeatureSelectionMode.REQUIRE_COUNTS
                ? ScenarioFeatureSet.RATIO_AND_COUNTS
                : ScenarioFeatureSet.RATIO_ONLY;
        String reason = countsPassed
                ? "COUNTS_CROSS_ENVIRONMENT_VALIDATED"
                : mode == FeatureSelectionMode.REQUIRE_COUNTS
                        ? "REQUIRED_COUNTS_GATE_FAILED"
                        : "COUNTS_GATE_FALLBACK_RATIO";
        return new Decision(new FeatureSelectionDecision(mode, selected, metrics, reason), contextPassed, countsPassed);
    }

    private static void addComparedMetrics(
            List<AblationMetric> destination,
            String kind,
            List<EvaluationSummary> baseline,
        ...
        for (Map.Entry<String, EvaluationSummary> entry : baselines.entrySet()) {
            ...
            int rows = right.scenarios().stream()
                    .mapToInt(ScenarioEvaluationMetrics::rowCount)
                    .sum();
            String target = right.scenarios().stream()
                    .map(metric -> metric.scenario().canonical())
                    .sorted()
                    .reduce((a, b) -> a + ";" + b)
                    .orElse("");
            destination.add(new AblationMetric(
                    kind,
                    entry.getKey(),
                    right.featureSet(),
                    left.featureSet(),
    ...
    private static AblationMetric aggregateMetric(
            String kind,
            String fold,
            ScenarioFeatureSet feature,
            ScenarioFeatureSet comparison,
            String target,
            Aggregate aggregate,
            double maeDelta,
            double spearmanDelta,
            boolean selected,
            String status,
            String reason) {
        return new AblationMetric(
                ...
                aggregate.mae(),
                aggregate.spearman(),
                optionalFinite(maeDelta),
                optionalFinite(spearmanDelta),
                selected,
                status,
                reason);
    }
    ...
    private static Aggregate aggregate(List<EvaluationSummary> folds) {
        if (folds.isEmpty()) {
            return new Aggregate(false, 0, OptionalDouble.empty(), OptionalDouble.empty(), OptionalDouble.empty());
        }
        boolean ok = folds.stream().allMatch(ScenarioAblationPlanner::allOk);
        int rows = folds.stream()
                .flatMap(fold -> fold.scenarios().stream())
                .mapToInt(ScenarioEvaluationMetrics::rowCount)
                .sum();
        OptionalDouble mae =
                average(folds.stream().map(EvaluationSummary::macroMae).toList());
        ...
        OptionalDouble worst = folds.stream().allMatch(fold -> fold.macroMae().isPresent())
                ? OptionalDouble.of(folds.stream()
                        .mapToDouble(fold -> fold.macroMae().getAsDouble())
                        .max()
                        .orElseThrow())
                : OptionalDouble.empty();
    ...
    private static boolean sameFoldIds(List<EvaluationSummary> baseline, List<EvaluationSummary> candidate) {
    ...
    private static OptionalDouble average(List<OptionalDouble> values) {
        ...
        for (OptionalDouble value : values) {
            ...
            correction += StrictMath.abs(sum) >= StrictMath.abs(addend) ? (sum - next) + addend : (addend - next) + sum;
            sum = next;
        }
    ...
    record Decision(FeatureSelectionDecision selection, boolean contextPassed, boolean countsPassed) {}

    private record Aggregate(
            boolean ok, int rows, OptionalDouble mae, OptionalDouble spearman, OptionalDouble worstMae) {}
}
