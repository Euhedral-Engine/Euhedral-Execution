...
final class ScenarioAblationPlanner {
    ...
    static Decision decide(FeatureSelectionMode mode, List<EvaluationSummary> policyContextFolds,
            List<EvaluationSummary> ratioContextFolds,
            List<EvaluationSummary> ratioEnvironmentFolds,
            List<EvaluationSummary> countEnvironmentFolds, int environmentCount,
        ...
        boolean contextMetrics =
                sameFoldIds(policyContextFolds, ratioContextFolds) && policy.ok() && ratio.ok()
                        && policy.mae().isPresent() && ratio.mae().isPresent() && policy.spearman()
                        .isPresent() && ratio.spearman().isPresent();
        double contextMaeDelta =
                contextMetrics ? ratio.mae().getAsDouble() - policy.mae().getAsDouble()
                        : Double.NaN;
        double contextSpearmanDelta =
                contextMetrics ? ratio.spearman().getAsDouble() - policy.spearman().getAsDouble()
                        : Double.NaN;
        boolean useful = contextMetrics && (ratio.mae().getAsDouble()
                <= policy.mae().getAsDouble() - thresholds.minimumContextMaeImprovement()
                || ratio.spearman().getAsDouble() >= policy.spearman().getAsDouble()
                + thresholds.minimumContextSpearmanImprovement());
        boolean contextPassed = useful && ratio.mae().getAsDouble()
                <= policy.mae().getAsDouble() + thresholds.maximumContextMaeRegression()
                && ratio.spearman().getAsDouble()
                >= policy.spearman().getAsDouble() - thresholds.maximumContextSpearmanRegression();
        metrics.add(aggregateMetric("VALIDATION_CONTEXT_GATE", "all", ScenarioFeatureSet.RATIO_ONLY,
                ScenarioFeatureSet.POLICY_ONLY, "all", ratio, contextMaeDelta, contextSpearmanDelta,
                contextPassed, contextMetrics ? contextPassed ? "PASS" : "FAIL" : "NOT_EVALUABLE",
        ...
        if (mode == FeatureSelectionMode.RATIO_ONLY) {
            return new Decision(
                    new FeatureSelectionDecision(mode, ScenarioFeatureSet.RATIO_ONLY, metrics,
                            "RATIO_ONLY_REQUESTED"), contextPassed, true);
        }
        if (environmentCount < 2) {
            ...
            metrics.add(new AblationMetric("VALIDATION_COUNTS_LOEO", "all",
                    ScenarioFeatureSet.RATIO_AND_COUNTS, ScenarioFeatureSet.RATIO_ONLY, "all", 0,
                    OptionalDouble.empty(), OptionalDouble.empty(), OptionalDouble.empty(),
                    OptionalDouble.empty(), false, "NOT_EVALUABLE", "INSUFFICIENT_ENVIRONMENTS"));
        ...
        boolean countMetrics = ratioEnvironmentFolds.size() == environmentCount && sameFoldIds(
                ratioEnvironmentFolds, countEnvironmentFolds) && ratioEnvironment.ok()
                && countsEnvironment.ok() && ratioEnvironment.mae().isPresent()
                && countsEnvironment.mae().isPresent() && ratioEnvironment.spearman().isPresent()
                && countsEnvironment.spearman().isPresent() && ratioEnvironment.worstMae()
                .isPresent() && countsEnvironment.worstMae().isPresent();
        double countMaeDelta =
                countMetrics ? countsEnvironment.mae().getAsDouble() - ratioEnvironment.mae()
                        .getAsDouble() : Double.NaN;
        double countSpearmanDelta = countMetrics ? countsEnvironment.spearman().getAsDouble()
                - ratioEnvironment.spearman().getAsDouble() : Double.NaN;
        double worstDelta = countMetrics ? countsEnvironment.worstMae().getAsDouble()
                - ratioEnvironment.worstMae().getAsDouble() : Double.NaN;
        boolean countsPassed = countMetrics && countsEnvironment.mae().getAsDouble()
        ...
        String reason = countsPassed ? "COUNTS_CROSS_ENVIRONMENT_VALIDATED"
                : mode == FeatureSelectionMode.REQUIRE_COUNTS ? "REQUIRED_COUNTS_GATE_FAILED"
                        : "COUNTS_GATE_FALLBACK_RATIO";
    ...
    private static void addComparedMetrics(List<AblationMetric> destination, String kind,
            List<EvaluationSummary> baseline, List<EvaluationSummary> candidate, boolean selected) {
        ...
        for (Map.Entry<String, EvaluationSummary> entry : baselines.entrySet()) {
            ...
            OptionalDouble spearmanDelta = delta(right.macroSpearman(), left.macroSpearman());
            int rows =
                    right.scenarios().stream().mapToInt(ScenarioEvaluationMetrics::rowCount).sum();
            String target =
                    right.scenarios().stream().map(metric -> metric.scenario().canonical()).sorted()
                            .reduce((a, b) -> a + ";" + b).orElse("");
            destination.add(
                    new AblationMetric(kind, entry.getKey(), right.featureSet(), left.featureSet(),
                            target, rows, right.macroMae(), right.macroSpearman(), maeDelta,
                            spearmanDelta, selected, allOk(right) ? "OK" : "FOLD_FAILED",
                            allOk(right) ? "FOLD_EVALUATED" : "NON_OK_SCENARIO"));
            destination.add(
                    new AblationMetric(kind, entry.getKey(), left.featureSet(), right.featureSet(),
                            target, rows, left.macroMae(), left.macroSpearman(),
    ...
    private static AblationMetric aggregateMetric(String kind, String fold,
        ...
        return new AblationMetric(kind, fold, feature, comparison, target, aggregate.rows(),
                aggregate.mae(), aggregate.spearman(), optionalFinite(maeDelta),
                optionalFinite(spearmanDelta), selected, status, reason);
    }
    ...
    private static Aggregate aggregate(List<EvaluationSummary> folds) {
        ...
        OptionalDouble worst =
                folds.stream().allMatch(fold -> fold.macroMae().isPresent()) ? OptionalDouble.of(
                        folds.stream().mapToDouble(fold -> fold.macroMae().getAsDouble()).max()
                                .orElseThrow()) : OptionalDouble.empty();
    ...
    private static TreeMap<String, EvaluationSummary> byFold(List<EvaluationSummary> values) {
        ...
        for (EvaluationSummary value : values) {
            String fold = value.scenarios().isEmpty() ? "" : value.scenarios().getFirst().foldId();
    ...
    private static boolean sameFoldIds(List<EvaluationSummary> baseline,
        ...
        return !baseline.isEmpty() && byFold(baseline).keySet().equals(byFold(candidate).keySet());
    }

    private static OptionalDouble delta(OptionalDouble candidate, OptionalDouble baseline) {
        return candidate.isPresent() && baseline.isPresent() ? OptionalDouble.of(
                candidate.getAsDouble() - baseline.getAsDouble()) : OptionalDouble.empty();
    }
    ...
    private static OptionalDouble average(List<OptionalDouble> values) {
        ...
        for (OptionalDouble value : values) {
            ...
            correction += StrictMath.abs(sum) >= StrictMath.abs(addend) ? (sum - next) + addend
                    : (addend - next) + sum;
            sum = next;
        }
    ...
    private record Aggregate(boolean ok, int rows, OptionalDouble mae, OptionalDouble spearman,
                             OptionalDouble worstMae) {
