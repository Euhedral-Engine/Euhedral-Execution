...
record TrainingHistoryEntry(String trainingKind, String foldId, ScenarioFeatureSet featureSet,
        int memberIndex, long memberSeed, int epoch, double validationMacroMae,
        OptionalDouble validationMacroSpearman, double validationWeightedBce,
        boolean selectedEpoch) implements Comparable<TrainingHistoryEntry> {
    ...
    public int compareTo(TrainingHistoryEntry other) {
        ...
        if (result == 0) result = foldId.compareTo(other.foldId);
        if (result == 0) result = featureSet.schemaId().compareTo(other.featureSet.schemaId());
        if (result == 0) result = Integer.compare(memberIndex, other.memberIndex);
