...
public record AblationMetric(
    ...
    private static void validate(OptionalDouble value, double minimum, double maximum, String name) {
        if (value.isPresent()
                && (!Double.isFinite(value.getAsDouble())
                        || value.getAsDouble() < minimum
                        || value.getAsDouble() > maximum)) {
