...
public final class CarryForwardQueue {
    ...
    public static List<CarryForwardEntry> reconcile(
        ...
        Map<PolicyId, PredictedPolicySummary> predictions = completedSchedule.selectedPredictions().stream()
        ...
        for (PolicyId admitted : completedSchedule.carryAdmissions()) {
            ...
            working.put(
                    admitted,
                    new CarryForwardEntry(
                            vector,
                            completedIteration,
                            completedIteration,
                            scenarioGrid(prediction, corpus, admitted)));
        }
        ...
        for (CarryForwardEntry entry : working.values()) {
            ...
            TreeMap<SourceScenario, CarryScenarioState> states = new TreeMap<>(entry.scenarios());
            for (var stateEntry : states.entrySet()) {
                ...
                CarryScenarioState updated = new CarryScenarioState(
                        scenario,
                        coverage,
                        state.attemptCount(),
                        state.lastAttemptIteration(),
                        coverage == CoverageState.VALID ? 0 : state.nextEligibleIteration(),
                        state.prediction());
                if (attempted
                        .getOrDefault(scenario, java.util.Set.of())
                        .contains(entry.policy().id())) {
                    updated = updated.attempted(completedIteration, coverage);
                }
                states.put(scenario, updated);
            }
            result.add(new CarryForwardEntry(entry.policy(), entry.firstSeenIteration(), completedIteration, states));
        }
    ...
    public static List<CarryForwardEntry> rescore(
            ...
            int iteration) {
        java.util.Map<
                        io.euhedral_execution.training.data.PolicyId,
                        io.euhedral_execution.training.optimization.data.PredictedPolicySummary>
                predictions = predictor
                        .predict(entries.stream().map(CarryForwardEntry::policy).toList())
                        .stream()
                        .collect(java.util.stream.Collectors.toMap(
                                summary -> summary.policy().id(), java.util.function.Function.identity()));
        return entries.stream()
                .map(entry -> {
                    PredictedPolicySummary summary =
                            predictions.get(entry.policy().id());
                    if (summary == null
                            || summary.predictions().size() != entry.scenarios().size()) {
                    ...
                    for (ScenarioPrediction prediction : summary.predictions()) {
                        ...
                        states.put(
                                prediction.scenario(),
                                new CarryScenarioState(
                                        prediction.scenario(),
                                        old.coverage(),
                                        old.attemptCount(),
                                        old.lastAttemptIteration(),
                                        old.nextEligibleIteration(),
                                        prediction));
                    }
                    return new CarryForwardEntry(entry.policy(), entry.firstSeenIteration(), iteration, states);
                })
                .toList();
    }

    public static List<CarryForwardEntry> selectForScenario(
            List<CarryForwardEntry> entries, SourceScenario scenario, int iteration, int limit) {
    ...
    private static Comparator<CarryForwardEntry> priority() {
        return Comparator.comparingInt(CarryForwardEntry::validScenarioCount)
                .reversed()
                .thenComparing((left, right) -> Double.compare(pessimisticMissing(right), pessimisticMissing(left)))
    ...
    private static SortedMapBuilder scenarioGrid(
        ...
        for (ScenarioPrediction row : prediction.predictions()) {
            builder.put(
                    ...
                    new CarryScenarioState(
                            row.scenario(),
                            coverage(corpus, policyId, row.scenario()),
                            0,
    ...
    private static CoverageState coverage(OptimizationCorpusView corpus, PolicyId policyId, SourceScenario scenario) {
        ScenarioResultStatus status = corpus.coverage()
                .getOrDefault(policyId, java.util.Collections.emptySortedMap())
                .get(scenario);
        if (status == ScenarioResultStatus.VALID_STRONG || status == ScenarioResultStatus.VALID_WEAK_OVERRIDE) {
