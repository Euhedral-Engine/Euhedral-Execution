...
public final class CandidateScheduler {
    ...
    public static SchedulePreparation prepare(
        ...
        BudgetAllocation allocation = BudgetAllocator.allocate(
                candidateBudget, calibrationPlan.anchors().fixedAnchors().size(), budgetConfig);
        List<PolicyVector> anchors = calibrationPlan.anchors().fixedAnchors();
        Set<PolicyId> anchorIds = anchors.stream().map(PolicyVector::id).collect(java.util.stream.Collectors.toSet());
        ...
        leaderPredictions.forEach(summary -> predictions.add(
                new ScheduledPolicyPrediction(summary.policy(), summary, SchedulePolicyOrigin.MEASURED_LEADER)));
        TreeMap<PolicyId, CarryForwardEntry> uniqueCarry = new TreeMap<>();
        carry.values()
                .forEach(rows ->
                        rows.forEach(entry -> uniqueCarry.put(entry.policy().id(), entry)));
        for (CarryForwardEntry entry : uniqueCarry.values()) {
            var curve = new PolicyPredictionCurve(
                    entry.policy(),
                    entry.scenarios().values().stream()
                            .map(CarryScenarioState::prediction)
                            .toList());
            predictions.add(new ScheduledPolicyPrediction(
                    entry.policy(),
                    PredictedPolicyRanker.summarize(
                            curve, new TreeSet<>(entry.scenarios().keySet())),
                    SchedulePolicyOrigin.MEASURED_CARRY));
        }
        int maximumKnownShortfall = selectedScenarios.stream()
                .mapToInt(scenario -> allocation.carryForward()
                        - carry.get(scenario).size()
                        + allocation.leaderRevalidation()
                        - leaders.size())
                .max()
                .orElse(0);
        return new SchedulePreparation(
                iteration,
                candidateBudget,
                allocation,
                selectedScenarios,
    ...
    public static IterationSchedule complete(
        ...
        ArrayList<ScheduledPolicyPrediction> predictions = new ArrayList<>(preparation.measuredPredictions());
        generated
                .disagreementAudits()
                .forEach(candidate -> predictions.add(new ScheduledPolicyPrediction(
                        candidate.policy(), candidate.prediction(), scheduleOrigin(candidate.origin()))));
        generated
                .baseExploration()
                .forEach(candidate -> predictions.add(new ScheduledPolicyPrediction(
                        candidate.policy(), candidate.prediction(), scheduleOrigin(candidate.origin()))));
        ...
        ArrayList<PolicyId> admissions = predictions.stream()
                ...
                .sorted((l, r) -> PredictedPolicyComparator.BEST_FIRST.compare(l.prediction(), r.prediction()))
        ...
        for (SourceScenario scenario : preparation.scenarios()) {
            ...
            BenchmarkParameters identityParameters = new BenchmarkParameters(
                    benchmarkConfig.expectedRepetitions(),
                    benchmarkConfig.sampleDurationNanos(),
                    benchmarkConfig.livenessTimeoutNanos(),
                    benchmarkConfig.framesPerSource(),
                    benchmarkConfig.resetTimeoutNanos(),
                    benchmarkConfig.orderedFrames(),
                    cpuSetHex,
                    java.util.stream.IntStream.range(0, scenario.sourceCount())
                            .mapToObj(index -> new FrameSourceSeed(index, 0, 0))
                            .toList());
            String runId = SchedulerSeeds.benchmarkRunId(
                    trainingRunId,
                    RunKind.NORMAL.name(),
                    preparation.iteration(),
                    scenario,
            ...
            List<FrameSourceSeed> seeds = java.util.stream.IntStream.range(0, scenario.sourceCount())
                    .mapToObj(index -> SchedulerSeeds.frameSourceSeed(runId, index, schedulerSeed))
                    .toList();
            BenchmarkParameters parameters = new BenchmarkParameters(
                    ...
                    cpuSetHex,
                    seeds);
            List<ScheduledPolicy> policies = orderPolicies(
                    rolePolicies, preparation.fixedAnchors(), cohort, schedulerSeed, preparation.candidateBudget());
            runs.add(new ScheduledRun(RunKind.NORMAL, scenario, runId, cohort, parameters, policies));
            ...
            reports.add(new ScenarioBudgetReport(
                    scenario,
                    preparation.candidateBudget(),
                    a.fixedAnchors(),
                    a.fixedAnchors(),
                    a.carryForward(),
                    carryAssigned,
                    ...
                    a.disagreementAudit() - auditAssigned,
                    policies.size()));
        }
        return new IterationSchedule(
                trainingRunId,
                preparation.iteration(),
                runs,
                predictions,
                admissions,
                reports,
                generated.nextSobolCursor());
    }

    private static List<RolePolicy> policiesForScenario(
            SchedulePreparation preparation, CandidateGenerationResult generated, SourceScenario scenario) {
        ArrayList<RolePolicy> rows = new ArrayList<>();
        preparation.fixedAnchors().forEach(policy -> rows.add(new RolePolicy(policy, PolicyRole.FIXED_ANCHOR)));
        preparation
                .carryByScenario()
                .getOrDefault(scenario, List.of())
                .forEach(entry -> rows.add(new RolePolicy(entry.policy(), PolicyRole.CARRY_FORWARD)));
        preparation
                .leaders()
                .forEach(leader -> rows.add(new RolePolicy(leader.policy(), PolicyRole.LEADER_REVALIDATION)));
        generated
                .disagreementAudits()
                .forEach(candidate -> rows.add(new RolePolicy(candidate.policy(), PolicyRole.DISAGREEMENT_AUDIT)));
        generated
                .baseExploration()
                .forEach(candidate -> rows.add(new RolePolicy(candidate.policy(), PolicyRole.EXPLORATION)));
        ...
        generated.overflowExploration().stream()
                .limit(overflowNeeded)
                .forEach(candidate -> rows.add(new RolePolicy(candidate.policy(), PolicyRole.EXPLORATION)));
    ...
    private static List<ScheduledPolicy> orderPolicies(
            List<RolePolicy> policies, List<PolicyVector> anchors, String cohort, long schedulerSeed, int budget) {
        Set<PolicyId> anchorIds = anchors.stream().map(PolicyVector::id).collect(java.util.stream.Collectors.toSet());
        List<RolePolicy> nonAnchors = policies.stream()
                .filter(row -> !anchorIds.contains(row.policy().id()))
                .sorted((left, right) -> {
                    int result = Long.compareUnsigned(
                            SchedulerSeeds.trialOrderKey(cohort, left.policy().id(), schedulerSeed),
                            SchedulerSeeds.trialOrderKey(cohort, right.policy().id(), schedulerSeed));
        ...
        List<RolePolicy> sortedAnchors = policies.stream()
                .filter(row -> anchorIds.contains(row.policy().id()))
                .sorted(Comparator.comparing(RolePolicy::policyId))
                .toList();
        for (int i = 0; i < sortedAnchors.size(); i++) {
            int position =
                    Math.toIntExact(Math.floorDiv(Math.multiplyExact(2L * i + 1L, budget), 2L * sortedAnchors.size()));
        ...
        for (int i = 0; i < budget; i++) {
            result.add(new ScheduledPolicy(i + 1, ordered[i].policy(), Set.of(ordered[i].role())));
        }
    ...
    private record RolePolicy(PolicyVector policy, PolicyRole role) implements SchedulerSeeds.PolicyWithRole {
        @Override
