...
import io.euhedral_execution.training.optimization.SchedulePolicyOrigin;
...
public final class ScheduleCodec {
    ...
    private static IterationSchedule read(Path directory,
        ...
        List<List<String>> runRows = Phase3Csv.read(root.resolve("runs.csv"));
    ...
    private static String runs(IterationSchedule schedule) {
        StringBuilder out = new StringBuilder(Phase3Csv.row(RUN_HEADER));
        for (ScheduledRun run : schedule.runs().stream()
            ...
            out.append(Phase3Csv.row(List.of("1", schedule.trainingRunId(),
    ...
    private static String predictions(IterationSchedule schedule) {
        StringBuilder out = new StringBuilder(Phase3Csv.row(PREDICTION_HEADER));
        for (ScheduledPolicyPrediction item : schedule.selectedPredictions().stream()
            ...
            for (ScenarioPrediction scenario : summary.predictions()) {
                out.append(Phase3Csv.row(List.of("1", item.policy().id().canonical(),
    ...
    private static String admissions(IterationSchedule schedule) {
        StringBuilder out = new StringBuilder(Phase3Csv.row(ADMISSION_HEADER));
        for (int i = 0; i < schedule.carryAdmissions().size(); i++) {
            out.append(Phase3Csv.row(List.of("1", Integer.toString(i + 1),
