...
public final class BootstrapScheduler {
    ...
    public static IterationSchedule create(
        ...
        for (int i = 0; i < bootstrapPolicies.size(); i++) {
            policies.add(new ScheduledPolicy(i + 1, bootstrapPolicies.get(i), Set.of(PolicyRole.EXPLORATION)));
        }
        List<BootstrapIdentity> identityPolicies = policies.stream()
                .map(policy -> new BootstrapIdentity(policy.policy().id(), PolicyRole.EXPLORATION))
                .toList();
        String cohort = SchedulerSeeds.candidateCohortId(
                trainingRunId, RunKind.BOOTSTRAP.name(), 0, scenario, identityPolicies, schedulerSeed);
        ...
        return new IterationSchedule(
                trainingRunId,
                0,
                List.of(new ScheduledRun(RunKind.BOOTSTRAP, scenario, run, cohort, parameters, policies)),
                List.of(),
                List.of(),
                List.of(new ScenarioBudgetReport(
                        scenario,
                        policies.size(),
                        0,
                        0,
    ...
    private record BootstrapIdentity(PolicyId policyId, PolicyRole role) implements SchedulerSeeds.PolicyWithRole {}
}
