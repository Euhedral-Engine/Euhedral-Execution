...
public final class ScheduleCodec {
    private static final List<String> FILES = List.of(
            "runs.csv", "policies.csv", "predictions.csv", "budget-report.csv", "carry-admissions.csv", "COMPLETE");
    private static final List<String> RUN_HEADER = List.of(
            ...
            "cpu_set_hex",
            "frame_source_seeds");
    private static final List<String> PREDICTION_HEADER = List.of(
            "schema_version",
            "policy_id",
            "scenario_id",
            "predicted_quality",
            ...
            "epistemic_stddev",
            "disagreement_range",
            "predicted_worst_quality",
            "predicted_quality_p25",
            "predicted_geometric_mean_quality",
            "predicted_quality_mad",
            "maximum_epistemic_stddev",
            "maximum_disagreement_range",
            "mean_ordinal_stddev",
            "mean_ordinal_entropy",
            "pessimistic_quality",
            "origin");
    private static final List<String> BUDGET_HEADER = List.of(
            "schema_version",
            "scenario_id",
            "candidate_budget",
            "fixed_requested",
            ...
            "exploration_assigned",
            "carry_transferred_to_exploration",
            "leader_transferred_to_exploration",
            "audit_transferred_to_exploration",
            "total_assigned");
    private static final List<String> ADMISSION_HEADER =
            List.of("schema_version", "predicted_rank", "policy_id", "first_seen_iteration");
    ...
    public static Path write(Path targetDirectory, IterationSchedule schedule) throws IOException {
        ...
        try {
            ...
            Files.writeString(temp.resolve("predictions.csv"), predictions(schedule), StandardCharsets.UTF_8);
            Files.writeString(temp.resolve("budget-report.csv"), budgets(schedule), StandardCharsets.UTF_8);
            Files.writeString(temp.resolve("carry-admissions.csv"), admissions(schedule), StandardCharsets.UTF_8);
            ...
            read(
                    temp,
                    catalog,
                    schedule.trainingRunId(),
                    0L,
                    "0".repeat(40),
                    false,
                    schedule.runs().isEmpty()
    ...
    private static IterationSchedule read(
            Path directory,
            SortedSet<SourceScenario> requiredScenarios,
            String expectedTrainingRunId,
            long schedulerSeed,
        ...
        for (int i = 1; i < runRows.size(); i++) {
            ...
            RunRow metadata = new RunRow(RunKind.valueOf(row.get(3)), scenario, row.get(5), row.get(6), parameters);
        ...
        Map<SourceScenario, List<ScheduledPolicy>> policies = readPolicies(root.resolve("policies.csv"), runMetadata);
        for (RunRow row : runMetadata.values()) {
            ...
            if (verifyIdentity) {
                validateIdentity(
                        expectedTrainingRunId, iteration, row, scheduled, schedulerSeed, commitSha, dirtyWorkingTree);
            }
            runs.add(new ScheduledRun(
                    row.kind(), row.scenario(), row.runId(), row.cohortId(), row.parameters(), scheduled));
        }
        TreeMap<PolicyId, PolicyVector> policyVectors = new TreeMap<>();
        policies.values()
                .forEach(list ->
                        list.forEach(policy -> policyVectors.put(policy.policy().id(), policy.policy())));
        List<ScheduledPolicyPrediction> predictions =
                readPredictions(root.resolve("predictions.csv"), requiredScenarios, policyVectors);
        List<ScenarioBudgetReport> budgets = readBudgets(root.resolve("budget-report.csv"), policies);
        List<PolicyId> admissions = readAdmissions(root.resolve("carry-admissions.csv"), iteration, predictions);
        validateScheduleSemantics(runs, predictions, admissions, budgets, requiredScenarios);
        return new IterationSchedule(expectedTrainingRunId, iteration, runs, predictions, admissions, budgets, 0);
    }
    ...
    private static Map<SourceScenario, List<ScheduledPolicy>> readPolicies(Path file, Map<SourceScenario, RunRow> runs)
            throws IOException {
        ...
        for (int i = 1; i < rows.size(); i++) {
            ...
            List<ScheduledPolicy> list = result.computeIfAbsent(scenario, ignored -> new ArrayList<>());
    ...
    private static List<ScheduledPolicyPrediction> readPredictions(
            Path file, SortedSet<SourceScenario> required, Map<PolicyId, PolicyVector> policies) throws IOException {
        List<List<String>> rows = CanonicalCsv.read(file);
        ...
        for (int i = 1; i < rows.size(); i++) {
            ...
            if (!required.contains(scenario)
                    || previousPolicy != null
                            && (policy.compareTo(previousPolicy) < 0
                                    || policy.equals(previousPolicy) && scenario.compareTo(previousScenario) <= 0)) {
            ...
            PredictionBuilder builder = builders.computeIfAbsent(
                    policy,
                    ignored ->
                            new PredictionBuilder(vector, rawSummary(row), SchedulePolicyOrigin.valueOf(row.get(20))));
    ...
    private static List<ScenarioBudgetReport> readBudgets(
            Path file, Map<SourceScenario, List<ScheduledPolicy>> policies) throws IOException {
        List<List<String>> rows = CanonicalCsv.read(file);
        ...
        for (int i = 1; i < rows.size(); i++) {
            ...
            previous = scenario;
            ScenarioBudgetReport report = new ScenarioBudgetReport(
                    scenario,
                    integer(row.get(2)),
                    integer(row.get(3)),
                    integer(row.get(4)),
    ...
    private static List<PolicyId> readAdmissions(Path file, int iteration, List<ScheduledPolicyPrediction> predictions)
            throws IOException {
        List<List<String>> rows = CanonicalCsv.read(file);
        requireHeader(rows, ADMISSION_HEADER);
        Map<PolicyId, PredictedPolicySummary> summaries = predictions.stream()
                .collect(java.util.stream.Collectors.toMap(
                        item -> item.policy().id(), ScheduledPolicyPrediction::prediction));
        ...
        for (int i = 1; i < rows.size(); i++) {
            ...
            if (summary == null
                    || previous != null && PredictedPolicyComparator.BEST_FIRST.compare(previous, summary) > 0
                    || !result.add(policy)) {
    ...
    private static void validateIdentity(
            ...
            String commitSha,
            boolean dirtyWorkingTree) {
        List<RoleIdentity> identities = policies.stream()
                .map(policy -> new RoleIdentity(
                        policy.policy().id(), policy.roles().iterator().next()))
                .toList();
        String cohort = SchedulerSeeds.candidateCohortId(
                trainingRunId, row.kind().name(), iteration, row.scenario(), identities, schedulerSeed);
    ...
    private static void validateTrialOrder(List<ScheduledPolicy> policies, String cohort, long schedulerSeed) {
        List<ScheduledPolicy> anchors = policies.stream()
                .filter(policy -> policy.roles().contains(PolicyRole.FIXED_ANCHOR))
                .sorted(Comparator.comparing(policy -> policy.policy().id()))
                .toList();
        int budget = policies.size();
        Set<Integer> anchorPositions = new HashSet<>();
        for (int i = 0; i < anchors.size(); i++) {
            int expected =
                    Math.toIntExact(Math.floorDiv(Math.multiplyExact(2L * i + 1L, budget), 2L * anchors.size())) + 1;
        ...
        for (ScheduledPolicy policy : policies) {
            ...
            long key = SchedulerSeeds.trialOrderKey(cohort, policy.policy().id(), schedulerSeed);
            if (previous != null) {
    ...
    private static void validateScheduleSemantics(
            List<ScheduledRun> runs,
            List<ScheduledPolicyPrediction> predictions,
            List<PolicyId> admissions,
            List<ScenarioBudgetReport> budgets,
        ...
        Set<PolicyId> predicted =
                predictions.stream().map(item -> item.policy().id()).collect(java.util.stream.Collectors.toSet());
    ...
    private static void validateBudget(ScenarioBudgetReport report, List<ScheduledPolicy> policies) {
        if (policies == null
                || report.totalAssigned() != report.candidateBudget()
                ...
                || report.explorationAssigned()
                        != report.explorationRequested()
                                + report.carryTransferredToExploration()
                                + report.leaderTransferredToExploration()
                                + report.auditTransferredToExploration()
                || report.carryRequested() - report.carryAssigned() != report.carryTransferredToExploration()
                || report.leaderRequested() - report.leaderAssigned() != report.leaderTransferredToExploration()
                || report.auditRequested() - report.auditAssigned() != report.auditTransferredToExploration()) {
    ...
    private static int count(List<ScheduledPolicy> policies, PolicyRole role) {
        return Math.toIntExact(policies.stream()
    ...
    private static String runs(IterationSchedule schedule) {
        ...
        for (ScheduledRun run : schedule.runs().stream()
                .sorted(Comparator.comparing(ScheduledRun::scenario))
                .toList()) {
            ...
            out.append(CanonicalCsv.row(List.of(
                    ...
                    p.cpuSetHex(),
                    encodeSeeds(p.frameSourceSeeds()))));
        }
        return out.toString();
    }
        ...
        for (ScheduledPolicyPrediction item : schedule.selectedPredictions().stream()
            ...
            for (ScenarioPrediction scenario : summary.predictions()) {
                out.append(CanonicalCsv.row(List.of(
                        ...
                        Double.toString(summary.pessimisticQuality()),
                        item.origin().name())));
            }
        }
        return out.toString();
    ...
    private static String budgets(IterationSchedule schedule) {
        ...
        for (ScenarioBudgetReport b : schedule.budgetReports().stream()
                .sorted(Comparator.comparing(ScenarioBudgetReport::scenario))
                .toList()) {
            out.append(CanonicalCsv.row(List.of(
                    "1",
                    b.scenario().canonical(),
                    Integer.toString(b.candidateBudget()),
                    Integer.toString(b.fixedRequested()),
                    Integer.toString(b.fixedAssigned()),
                    Integer.toString(b.carryRequested()),
                    Integer.toString(b.carryAssigned()),
                    Integer.toString(b.leaderRequested()),
                    Integer.toString(b.leaderAssigned()),
                    Integer.toString(b.auditRequested()),
                    Integer.toString(b.auditAssigned()),
    ...
    private static List<FrameSourceSeed> seeds(String value) {
        ...
        if (!value.isEmpty()) {
            for (String item : value.split(";")) {
                ...
                result.add(new FrameSourceSeed(integer(fields[0]), hex(fields[1]), hex(fields[2])));
            }
        }
        return List.copyOf(result);
    }
    ...
    private static void requireConfig(
            BenchmarkParameters parameters, BenchmarkExecutionConfig config, SourceScenario scenario) {
    ...
    private static BenchmarkExecutionConfig config(BenchmarkParameters parameters) {
        return new BenchmarkExecutionConfig(
                parameters.expectedRepetitions(),
                parameters.sampleDurationNanos(),
                parameters.livenessTimeoutNanos(),
                parameters.framesPerSource(),
                parameters.resetTimeoutNanos(),
    ...
    private static EnumSet<PolicyRole> roles(String value) {
        ...
        String canonical = result.stream()
                .map(Enum::name)
                .sorted()
                .reduce((left, right) -> left + ";" + right)
                .orElseThrow();
    ...
    private record RunRow(
            RunKind kind, SourceScenario scenario, String runId, String cohortId, BenchmarkParameters parameters) {}
    ...
    private static final class PredictionBuilder {
        ...
        private PredictionBuilder(PolicyVector policy, double[] summary, SchedulePolicyOrigin origin) {
            this.policy = policy;
        ...
        private void add(List<String> row, SourceScenario scenario) {
            ...
            scenarios.add(new ScenarioPrediction(
                    scenario,
                    finite(row.get(3)),
                    finite(row.get(4)),
                    finite(row.get(5)),
        ...
        private ScheduledPolicyPrediction build(SortedSet<SourceScenario> required) {
            ...
            PredictedPolicySummary recomputed =
                    PredictedPolicyRanker.summarize(new PolicyPredictionCurve(policy, scenarios), required);
            double[] actual = {
                recomputed.predictedWorstQuality(),
                ...
                recomputed.meanOrdinalStdDev(),
                recomputed.meanOrdinalEntropy(),
