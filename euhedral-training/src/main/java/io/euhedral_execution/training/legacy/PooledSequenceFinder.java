...
public class PooledSequenceFinder implements AutoCloseable {

    private static final Logger LOGGER = LoggerFactory.getLogger(PooledSequenceFinder.class);
    ...
    public PooledSequenceFinder() throws Exception {
    ...
    private PooledSequenceFinder(Path model) throws Exception {
        this.learner = loadOrCreate(model);
    }

    public static Path train(Path data, Path startingModel, Path modelOutput) throws Exception {
        try (PooledSequenceFinder finder = new PooledSequenceFinder(startingModel)) {
    ...
    public static Path generateCandidates(Path historicalData, Path model, Path output,
        ...
        try (PooledSequenceFinder finder = new PooledSequenceFinder(model)) {
