...
public final class SchedulingFixtures {
    ...
    public static PredictedPolicySummary prediction(PolicyVector policy, double... qualities) {
        ...
        for (SourceScenario scenario : SCENARIOS) {
            ...
            rows.add(new ScenarioPrediction(scenario, quality, 0.05,
                    StrictMath.max(0, quality - 0.1), StrictMath.min(1, quality + 0.1),
                    0.25, quality, 0.03, 0.06));
        }
        return PredictedPolicyRanker.summarize(new PolicyPredictionCurve(policy, rows),
                SCENARIOS);
    }

    public static PolicyCurvePredictor predictor() {
        return policies -> policies.stream().map(policy -> {
            double base = 0.35 + (Long.remainderUnsigned(policy.id().value(), 50) / 100.0);
            return prediction(policy, base, StrictMath.min(0.99, base + 0.05),
                    StrictMath.min(0.99, base + 0.10));
        }).toList();
    }

    public static RobustPolicySummary eligible(PolicyVector policy, double quality) {
        return new RobustPolicySummary(policy, true, SCENARIOS.size(), SCENARIOS.size(),
                SCENARIOS.size(), 1.0, OptionalDouble.of(quality),
                OptionalDouble.of(quality), OptionalDouble.of(quality),
                OptionalDouble.of(0.0), OptionalDouble.of(0.05),
                OptionalDouble.of(0.0), OptionalDouble.of(0.0), SCENARIOS,
                new TreeSet<>(), new TreeSet<>());
    }

    public static RobustPolicySummary incomplete(PolicyVector policy,
            SortedSet<SourceScenario> measured) {
        TreeSet<SourceScenario> missing = new TreeSet<>(SCENARIOS);
        missing.removeAll(measured);
        return new RobustPolicySummary(policy, false, SCENARIOS.size(), measured.size(),
                measured.size(), measured.size() / (double) SCENARIOS.size(),
                OptionalDouble.empty(), OptionalDouble.empty(), OptionalDouble.empty(),
                OptionalDouble.empty(), OptionalDouble.empty(), OptionalDouble.empty(),
                OptionalDouble.empty(), measured, missing, new TreeSet<>());
    }
    ...
    public static OptimizationCorpusView corpus(List<RobustPolicySummary> summaries) {
        ...
        for (RobustPolicySummary summary : summaries) {
            ...
            for (SourceScenario scenario : SCENARIOS) {
                rows.put(scenario, summary.measuredScenarios().contains(scenario)
                        ? ScenarioResultStatus.VALID_STRONG : ScenarioResultStatus.MISSING);
            }
            coverage.put(summary.policy().id(), rows);
        }
        return new OptimizationCorpusView(policies, summaries.stream()
                .filter(RobustPolicySummary::eligible).toList(), byId, coverage,
                "1".repeat(64));
    }
    ...
    public static CalibrationPlan calibration(List<PolicyVector> anchors) {
        ...
        return new CalibrationPlan(catalog, new ReferenceRunCatalog(1,
                catalog.anchorSetId(), references));
    }
