...
class CandidateSchedulerTest {
    private static List<PredictedCandidate> candidates(int start, int count, CandidateOrigin origin) {
        ArrayList<PredictedCandidate> result = new ArrayList<>();
        for (int i = 0; i < count; i++) {
            var policy = SchedulingFixtures.policy(start + i);
            result.add(new PredictedCandidate(
                    policy,
                    SchedulingFixtures.prediction(policy, 0.5 + i * 0.01, 0.6 + i * 0.01, 0.7 + i * 0.01),
                    origin));
        }
        return result;
    }
    ...
    void fillsExactDisjointBudgetsAndSpreadsAnchors() {
        ...
        IterationSchedule schedule = CandidateScheduler.complete(
                "training",
                77L,
                "0".repeat(40),
                false,
        ...
        for (ScheduledRun run : schedule.runs()) {
            ...
            assertThat(run.policies().stream()
