...
class ScoreBandSamplerTest {
    ...
    private static List<PolicyId> expectedIds(List<PredictedCandidate> candidates, int iteration, int limit) {
    ...
    private static PredictedPolicySummary summary(double[] vector, double quality) {
        ...
        ScenarioPrediction prediction =
                new ScenarioPrediction(scenario, quality, 0.0, quality, quality, 0.0, 0.0, 0.0, 0.0);
        return new PredictedPolicySummary(
                new PolicyPredictionCurve(policy, List.of(prediction)),
                quality,
                quality,
                quality,
                0.0,
    ...
    void retainsTheConfiguredNumberFromEveryScoreBand() {
        ...
        ScoreBandSampler sampler = new ScoreBandSampler(100, new int[] {1, 1, 1, 1, 2, 2, 3, 5, 8, 16}, 123L, 0, 100);
        ...
        for (int i = 0; i < 10_000; i++) {
            ...
            sampler.accept(new PredictedCandidate(summary.policy(), summary, CandidateOrigin.SCORE_BAND));
        }
        ...
        for (int band = 0; band < 10; band++) {
            ...
            assertThat(selected.stream()
                            .filter(candidate ->
                                    ScoreBandSampler.band(candidate.prediction().predictedWorstQuality())
                                            == currentBand)
                            .count())
                    .isEqualTo(expected);
        }
    ...
    void samplingKeysIncludeIteration() {
        ...
        assertThat(ids(first.finish())).containsExactlyElementsOf(expectedIds(candidates, 1, 4));
        assertThat(ids(second.finish())).containsExactlyElementsOf(expectedIds(candidates, 2, 4));
