...
class RunCalibratorTest {
    ...
    private final List<PolicyVector> anchors = List.of(policy(1), policy(2), policy(3),
            policy(4), policy(5));
    ...
    void removesGlobalTwoTimesScaleAndIgnoresCandidateCohort() {
        ...
        var calibration = calibration(withReferences(first)).stream()
                .filter(row -> row.run().descriptor().benchmarkRunId().equals("run-2")).findFirst()
                .orElseThrow();
    ...
    void stableMajorityAndWeightCapDefeatOutlier() {
        var equal = calibration(withReferences(runs("run-2",
                new double[]{2, 2, 2, 2, 8}, 1000, true))).getLast();
        ...
        for (int i = 0; i < anchors.size(); i++) {
            ...
            values.add(aggregate(anchors.get(i), "reference", scenario, reference, 100,
                    0, 0, 0, CalibrationRole.CANDIDATE, START));
        }
        for (int i = 0; i < anchors.size() - 1; i++) {
            double[] stable = new double[51];
            Arrays.fill(stable, referenceValues[i] * 2);
            values.add(aggregate(anchors.get(i), "run-3", scenario, stable, 100, 49, 0, 0,
                    CalibrationRole.ANCHOR, START.plusSeconds(10)));
        }
        double[] precise = new double[100];
        Arrays.fill(precise, 12_800);
        values.add(aggregate(anchors.getLast(), "run-3", scenario, precise, 100, 0, 0, 0,
                CalibrationRole.ANCHOR, START.plusSeconds(10)));
        var weighted = calibration(values).getLast();
        assertThat(weighted.cappedAnchorWeights().values()).allMatch(weight -> weight <= 0.25
                || weight <= Math.nextUp(0.25));
    ...
    void classifiesWeakInsufficientAndExcessiveResidual() {
        List<RunAggregate> four = withReferences(runs("run-four",
                new double[]{2, 2, 2, 2}, 0, false));
        assertThat(calibration(four).getLast().status())
                .isEqualTo(CalibrationStatus.WEAKLY_CALIBRATED);

        List<RunAggregate> two = withReferences(runs("run-two",
                new double[]{2, 2}, 0, false));
        ...
        List<RunAggregate> excessiveRuns = withReferences(runs("run-wide",
                new double[]{1, 1.3, 1.6, 2, 2.5}, 0, false));
        PolicyVector candidate = policy(30);
        excessiveRuns.add(aggregate(candidate, "run-wide", scenario, repeats(500), 5,
                0, 0, 0, CalibrationRole.CANDIDATE, START.plusSeconds(10)));
        ...
        var candidateScenario = HierarchicalAggregator.aggregateScenarios(List.of(candidate),
                excessiveRuns, excessiveCalibrations, new TreeSet<>(Set.of(scenario)),
                AggregationConfig.defaults()).getFirst();
        assertThat(candidateScenario.status())
                .isEqualTo(MergeRecords.ScenarioResultStatus.NO_ACCEPTED_CALIBRATION);
    }

    @Test
    void requiresFixedAnchorRoleOutsideReferenceAndCalibratesDirectly() {
        List<RunAggregate> all = withReferences(runs("middle",
                new double[]{2, 2, 2, 2, 2}, 0, false));
        all.addAll(runs("final", new double[]{4, 4, 4, 4, 4}, 0, false));
        RunAggregate wrongRole = aggregate(anchors.getLast(), "final", scenario,
                repeats(6400), 5, 0, 0, 0, CalibrationRole.CANDIDATE,
                START.plusSeconds(10));
        all.removeIf(row -> row.run().descriptor().benchmarkRunId().equals("final")
                && row.policy().id().equals(anchors.getLast().id()));
        all.add(wrongRole);
        var finalRun = calibration(all).stream().filter(row ->
                row.run().descriptor().benchmarkRunId().equals("final")).findFirst().orElseThrow();
    ...
    void preservesObservedNonRequiredScenarioWithoutInventingReference() {
        ...
        List<RunAggregate> rows = withReferences(List.of(aggregate(policy(40), "extra-run",
                extra, repeats(500), 5, 0, 0, 0, CalibrationRole.CANDIDATE,
                START.plusSeconds(20))));
        var extraCalibration = calibration(rows).stream().filter(item ->
                item.run().descriptor().benchmarkRunId().equals("extra-run"))
    ...
    private List<RunAggregate> withReferences(List<RunAggregate> current) {
        ...
        for (int i = 0; i < anchors.size(); i++) all.add(aggregate(anchors.get(i), "reference",
                scenario, repeats(referenceValues[i]), 5, 0, 0, 0, CalibrationRole.CANDIDATE, START));
        all.addAll(current);
        return all;
    }

    private List<RunAggregate> runs(String runId, double[] ratios, double candidate,
            boolean addCandidate) {
        List<RunAggregate> rows = new ArrayList<>();
        for (int i = 0; i < ratios.length; i++) rows.add(aggregate(anchors.get(i), runId,
                scenario, repeats(referenceValues[i] * ratios[i]), 5, 0, 0, 0,
                CalibrationRole.ANCHOR, START.plusSeconds(10)));
        if (addCandidate) rows.add(aggregate(policy(20), runId, scenario, repeats(candidate), 5,
                0, 0, 0, CalibrationRole.CANDIDATE, START.plusSeconds(10)));
        return rows;
    }
    ...
    private List<MergeRecords.RunCalibration> calibration(List<RunAggregate> rows) {
        ...
        return RunCalibrator.calibrate(rows, new CalibrationPlan(catalog,
                new ReferenceRunCatalog(1, catalog.anchorSetId(),
                        new TreeMap<>(Map.of(scenario, "reference")))),
