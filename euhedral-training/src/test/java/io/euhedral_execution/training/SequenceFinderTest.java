...
class SequenceFinderTest {
    ...
    void streamsFullScreenTransfersCmaShortfallAndAdvancesCursorDeterministically() {
        CandidateGenerationConfig config = new CandidateGenerationConfig(
                32, 7, new int[] {1, 1, 1, 1, 2, 2, 3, 5, 8, 16}, 8, 7, 1, new CmaEsConfig(false, 1, 1, 8, 0.2, 2));
        var corpus = SchedulingFixtures.corpus(List.of());
        CandidateGenerationRequest request = new CandidateGenerationRequest(
                1, 16, 4, 3, 100, 77L, corpus, Set.of(), SchedulingFixtures.predictor(), config);
        ...
        assertThat(java.util.stream.Stream.of(
                                first.disagreementAudits(), first.baseExploration(), first.overflowExploration())
                        .flatMap(List::stream)
