...
class ScenarioTrainingConfigTest {
    ...
    void recognizesResolvedAutoBatchAsTheEffectiveRequestedConfiguration() {
        ...
        assertThat(ScenarioTrainingConfig.defaults().isEffectiveVersionOf(requested))
                .isFalse();
    }

    @Test
    void rejectsAnEffectiveBatchLargerThanAnExplicitRequest() {
        ScenarioTrainingConfig requested =
                withBatchSize(ScenarioTrainingConfig.defaults().coldStart(), 128);

        assertThat(withBatchSize(requested, 64).isEffectiveVersionOf(requested)).isTrue();
        assertThat(withBatchSize(requested, 256).isEffectiveVersionOf(requested))
                .isFalse();
    }
}
