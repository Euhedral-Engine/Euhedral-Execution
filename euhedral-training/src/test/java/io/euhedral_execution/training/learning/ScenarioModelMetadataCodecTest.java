...
class ScenarioModelMetadataCodecTest {
    ...
    void roundTripsCanonicalBytesAndEveryRawNormalizerBit() throws Exception {
        ...
        assertThat(encoded).contains("\"artifact_type\": "
    ...
    void goldenMetadataDecodesAndCanonicalizesByteStably() throws Exception {
        ...
        try (InputStream input = getClass().getResourceAsStream(
                "/robust-training/v1/scenario-model-metadata.json")) {
        ...
        assertThat(ScenarioModelMetadataCodec.encode(
                ScenarioModelMetadataCodec.decode(canonical))).isEqualTo(canonical);
    }
    ...
    void rejectsUnknownDuplicateMissingAndTrailingFields() {
        ...
        assertThatThrownBy(() -> ScenarioModelMetadataCodec.decode(
                encoded.replaceFirst("\\{", "{\"unknown\": 1,")))
                .isInstanceOf(IOException.class);
        assertThatThrownBy(() -> ScenarioModelMetadataCodec.decode(
                encoded.replaceFirst("\"schema_version\": 1,",
                        "\"schema_version\": 1,\"schema_version\": 1,")))
                .isInstanceOf(IOException.class);
        assertThatThrownBy(() -> ScenarioModelMetadataCodec.decode(
                encoded.replaceFirst("\"policy_width\": 28,", "")))
                .isInstanceOf(IOException.class);
        assertThatThrownBy(() -> ScenarioModelMetadataCodec.decode(encoded + "x"))
                .isInstanceOf(IOException.class);
        assertThatThrownBy(() -> ScenarioModelMetadataCodec.decode(
                "{\"x\":\"\\\\q\"}")).isInstanceOf(IOException.class);
    }

    @Test
    void rejectsChangedObjectiveFeatureNamesAndThresholds() {
        String encoded = ScenarioModelMetadataCodec.encode(metadata());
        assertThatThrownBy(() -> ScenarioModelMetadataCodec.decode(encoded.replace(
                "scenario-quality-ordinal-v1", "scenario-quality-ordinal-v2")))
                .isInstanceOf(IOException.class);
        assertThatThrownBy(() -> ScenarioModelMetadataCodec.decode(encoded.replace(
                "\"schema_version\": 1", "\"schema_version\": 2")))
                .isInstanceOf(IOException.class);
        assertThatThrownBy(() -> ScenarioModelMetadataCodec.decode(encoded.replace(
                "\"feature_width\": 29", "\"feature_width\": 28")))
                .isInstanceOf(IOException.class);
        assertThatThrownBy(() -> ScenarioModelMetadataCodec.decode(encoded.replace(
                "policy_weight_00", "policy_weight_xx")))
                .isInstanceOf(IOException.class);
    ...
    static ScenarioModelMetadata metadata() {
        ...
        ScenarioLearningRow row = new ScenarioLearningRow(policy, scenario,
                ScenarioResultStatus.VALID_STRONG, 0.5, 10, 9, 11, 1, 0.1, 0);
        FeatureNormalizer normalizer = FeatureNormalizer.fit(
                List.of(row), ScenarioFeatureSet.RATIO_ONLY);
        ...
        TreeMap<String, java.util.SortedMap<SourceScenario, Integer>> scenarioCounts =
                new TreeMap<>();
        ...
        PartitionCounts partitions =
                new PartitionCounts(policyCounts, rowCounts, scenarioCounts);
        FeatureSelectionDecision selection = new FeatureSelectionDecision(
                ...
                List.of(new AblationMetric("VALIDATION_CONTEXT_GATE", "all",
                        ScenarioFeatureSet.RATIO_ONLY, ScenarioFeatureSet.POLICY_ONLY,
                        "all", 10, OptionalDouble.of(0.19), OptionalDouble.of(0.48),
                        OptionalDouble.of(-0.01), OptionalDouble.of(-0.02), true,
                        "PASS", "CONTEXT_VALIDATED")), "RATIO_ONLY_REQUESTED");
        EvaluationSummaryMetadata summary = new EvaluationSummaryMetadata(
                "grouped-evaluation.csv", "loso-evaluation.csv",
        ...
        List<MemberMetadata> members = java.util.stream.IntStream.range(0, 3)
                .mapToObj(index -> member(index, ScenarioMemberSeeds.derive(
                        config.modelSeed(), "PRODUCTION", ScenarioFeatureSet.RATIO_ONLY,
                        "all", index)))
                .toList();
        MetadataProbe probe = new MetadataProbe(policy.id(), scenario,
                List.of("0000000000000000", "0000000000000000",
                        "0000000000000000", "0000000000000000",
                        "0000000000000000", "0000000000000000",
                        "0000000000000000", "0000000000000000"), "cpu");
        return new ScenarioModelMetadata(1, ScenarioModelMetadata.OBJECTIVE_VERSION,
                ScenarioFeatureSet.RATIO_ONLY, normalizer,
                ...
                ScenarioModelMetadata.SPLIT_ALGORITHM, 1, 2, "a".repeat(64), false,
                new TreeSet<>(List.of(scenario)), new TreeSet<>(List.of(scenario)),
                partitions, config, selection, summary, ModelAcceptanceStatus.ACCEPTED,
                List.of(), new ProducerMetadata("0".repeat(40), false,
                "PyTorch", "2.7.1", "cpu"), probe);
    }

    private static MemberMetadata member(int index, long seed) {
        return new MemberMetadata(index, seed, 1, MemberMetadata.expectedPath(index),
