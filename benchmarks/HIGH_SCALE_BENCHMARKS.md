...
These are results for one CPU-heavy, fine-grained workload, not a claim that one scheduler will
have the same advantage everywhere. The comparison, scheduling setup, and known limitations are
documented below.
...
Euhedral receives the pre-allocated frames through a subscribed `EuhedralSubscriber`, then adds
that source to the control plane:
