...
public class UpstreamQueue {
    ...
    private double decayExposure;
    private long decayTimestamp;
    private long activeHalfLifeNanos;
    ...
    /// Owner-local prospective aging. The supplied initial H is restored after reset.
    public long getAdaptiveContention(long nowNs, long initialHalfLifeNanos) {
        if (initialHalfLifeNanos <= 0L) {
            throw new IllegalArgumentException("contention half-life must be positive");
        }
        if (this.activeHalfLifeNanos == 0L) {
            this.activeHalfLifeNanos = initialHalfLifeNanos;
        }
        if (this.contentionEvidenceCount == 0L) {
            return 0L;
        }
        if (this.contentionEvidenceCount != this.appliedContentionEvidenceCount) {
            this.appliedContentionEvidenceCount = this.contentionEvidenceCount;
            this.lastContentionEvidenceNanos = nowNs;
            this.decayTimestamp = nowNs;
            this.decayExposure = 0.0;
        } else {
            long elapsed = nowNs - this.decayTimestamp;
            if (elapsed > 0L) {
                this.decayExposure += (double) elapsed / this.activeHalfLifeNanos;
                this.decayTimestamp = nowNs;
            }
        }
        long stored = MathFunctions.clampLong(this.acquireContention.value(), 0L, ACQUIRE_CONTENTION_SCALE);
        return Math.round(stored * Math.exp(-LN_2 * this.decayExposure));
    }

    /// Account for elapsed time under the old H before installing the next segment's H.
    public void installContentionHalfLife(long nowNs, long halfLifeNanos, long initialHalfLifeNanos) {
        if (halfLifeNanos <= 0L) {
            throw new IllegalArgumentException("contention half-life must be positive");
        }
        getAdaptiveContention(nowNs, initialHalfLifeNanos);
        this.activeHalfLifeNanos = halfLifeNanos;
    }
    ...
    public void resetAcquireContention() {
        ...
        this.decayExposure = 0.0;
        this.decayTimestamp = 0L;
        this.activeHalfLifeNanos = 0L;
