...
| Implementation validation and blueprint-conformance audit                               | Strong coding/audit model, raised when the acceptance surface remains coupled | Medium to high                                                                     |
...
| Validation        | Pull request    | Evidence that the implementation satisfies its contract   |
...
non-frontier implementation agent. Completion records, validation evidence, and audits then flow
...
2. Blueprint
3. Implementation
4. Validation, blueprint-conformance audit, and handoff when the feature needs them

A phase consumes artifacts from earlier phases and records newly discovered conclusions in its own
artifact. Within a phase, prompts are small action items: blueprint, implementation, validation,
and audit action items may exist for each independently deliverable chunk.
...
to least reasoning-intensive, label every blueprint, validation, and audit prompt with its exact
model and reasoning effort, and give each implementation prompt a clearly labeled provisional
model and effort. Pair every blueprint with implementation, validation, and conformance-audit
...
responsibility, package ownership, inputs and outputs, acceptance criteria, and validation action
...
alternative; do not silently downgrade. Do not assume that validation or audit can repair a
...
## Phase 4: validation and blueprint-conformance audit
...
> production code. Produce a ranked, phased blueprint/implementation/validation/audit prompt
